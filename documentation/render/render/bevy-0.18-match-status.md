...
| N33 | Camera-component extraction mechanism (ACCEPTED, EQUIVALENT) | Bevy extracts `CameraMainTextureUsages`/`Msaa` etc. via `ExtractComponentPlugin`, while Epix hand-pulls them inside `extract_cameras`. Same end state (the component lands on the view entity); structurally different. Left as-is per the "render module makes nearly no change" rule (not a behavioral mismatch). |
| N34 | TemporaryRenderEntity in other extract systems (ACCEPTED, TO AUDIT) | Visible-entity/mesh2d/sprite/text extraction may still spawn `TemporaryRenderEntity` / omit `RenderEntity` where Bevy threads the synced entity through. Behavioral effect is equivalent for those renderables; a fuller audit is deferred rather than rewriting the working extractors. |
