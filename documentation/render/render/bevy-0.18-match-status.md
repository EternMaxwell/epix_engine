...
| N16 | AsBindGroup error model | OPEN | Epix still uses one `enum class AsBindGroupError` for GPU-asset creation and bind-group preparation; image/storage preparation return it and normal/erased render-asset systems log it by integer cast. Bevy's bind-group error is a tagged union of retry-next-update, create-bind-group-directly, and invalid-sampler-type (with binding index and descriptions). GPU asset-creation failures need a separate error type; callers must preserve and log the tagged error rather than coerce it to an integer. |
| N17 | AsBindGroup default preparation | OPEN | Epix requires every specialization to implement both `unprepared_bind_group` and `as_bind_group`. Bevy supplies the latter from the unprepared result using its label and cached pipeline layout, and also provides label/layout/layout-descriptor helpers. Add a C++-idiomatic default helper which materializes `BindingResources::entries()` and retains ordered ownership. Use `PipelineServer` for the intentional cache divergence; omit force-no-bindless/bindless descriptor paths under R1. |
| N18 | AsBindGroup shader-type conversion | OPEN | Bevy's `AsBindGroupShaderType<T>` conversion extension is absent. It converts a source value into a shader type with access to render image assets. Epix's equivalent must take `const RenderAssets<image::Image>&` (whose processed value is `GpuImage`) and return a `ShaderType`; the default constrained `const T& -> ShaderT` conversion does not require the images argument. This is unrelated to unsupported bindless support. |
