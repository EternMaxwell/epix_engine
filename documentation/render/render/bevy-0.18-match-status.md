...
| N12 | Render mesh allocation and morph support | TRACKED IN MESH | Mesh parity is now active. The allocator, render-mesh metadata, and morph-target gaps are tracked as M1–M8 in `documentation/mesh-core-graph-bevy-0.18-match-status.md`; they require mesh/core-pipeline work rather than a new render-module design. |
...
| N28 | Binned phase add parameters | FIXED | `BinnedRenderPhase::add` now takes its representative render/main identity as one `std::pair`, the direct C++ representation of Bevy's `(Entity, MainEntity)` tuple. The mesh queue, binned visual example, and focused render tests use that atomic pair. |
| N29 | Render-graph startup race | FIXED | Root cause: `task::Scope::spawn` resized its results vector via `m_results->resize(idx + 1)` WITHOUT holding the results mutex, while still-running task threads wrote their result slot under that mutex. When a later spawn's resize reallocated the vector, an in-flight task wrote into the freed buffer, corrupting the collected `CommandBuffer` handle — surfacing as an intermittent wgpu `invalid command buffer` abort in `wgpuQueueSubmit` (startup-only, more likely on heavy scenes whose longer task runtime widens the overlap). Fix: `Scope::spawn` now holds the results mutex across the resize, so task writes and growth are serialized. Verified: the heavy `examples_header_mesh_batching` scene ran 10×20s with **0 panics** (previously ~1–2 per 12 runs) with parallel generation restored. Added a task-module regression test `Scope.ConcurrentGrowthPreservesEveryResult`; note that a functional test cannot deterministically reproduce this data race — a sanitizer (AddressSanitizer/ThreadSanitizer) build is needed for deterministic detection, which the build does not yet expose. |
| N30 | Camera extraction onto synced render entity | FIXED | `extract_cameras` now queries `(Entity, RenderEntity, &Camera, ...)` and inserts `ExtractedCamera`/`ExtractedView`/etc. onto the camera's **synced render entity** (Bevy `camera.rs`), instead of spawning a separate `TemporaryRenderEntity`. This keeps the extracted camera components (e.g. `ExtractComponentPlugin<Tonemapping>`) and `ViewTarget` on one entity, matching Bevy's single-entity camera view. It also removes the extracted camera view components when inactive/invalid (Bevy `remove::<ExtractedCameraComponents>`). Commit `a1850464`. |
| N31 | RenderEntity queried by value | FIXED | Added `WorldQuery<RenderEntity>` + `QueryData<RenderEntity>` (Item = `RenderEntity`, read-only copy) so queries use `RenderEntity` **by value** like Bevy's `Query<(Entity, RenderEntity, ...)>`, rather than `const RenderEntity&`. `extract_cameras` and its mapper query switched to it. Commit `62af1b35`. |
| N32 | NoIndirectDrawing insert/remove | FIXED | `extract_cameras` now both `insert`s and `remove`s `view::NoIndirectDrawing` based on the culling-support condition (Bevy `insert`/`else remove`), so a camera that loses the condition no longer keeps a stale marker. Commit `36e4f4b4`. |
| N33 | Camera-component extraction mechanism (OPEN) | Bevy uses ExtractComponentPlugin for CameraMainTextureUsages/Msaa; Epix hand-pulls them inside extract_cameras. Re-audit lifecycle and match plugin ownership. The earlier ACCEPTED label was not user-approved; parent-module changes require review, not silent exemption. |
| N34 | TemporaryRenderEntity in other extract systems (REVIEW) | Audit synced render-entity usage in visible-entity/mesh2d/sprite/text extraction against the corresponding Bevy systems before claiming equivalence. The earlier ACCEPTED label and blanket equivalent-behavior claim were unsupported. |
| N35 | `prepare_uniform_components` accessed the component via `.get()` | C3's `FullscreenMaterial` (the first real `UniformComponentPlugin` consumer) exposed a latent bug: `prepare_uniform_components` called `component.get()` on the `Item<const C&>` query item, which is the raw `const C&` (the change-detection `Ref` is unwrapped by `QueryData::fetch`). It only compiled before because the test probe `UniformPluginProbe` coincidentally had a `.get()` member returning a copy. Fixed to push the component reference directly (`uniforms.push(component)`). Bevy's equivalent reads the `Ref` value; Epix's query item is already the un-wrapped value. |
| N36 | Unit-test module ownership (FIXED, pending commit review) | Moved standalone camera/projection/visibility, window-reference, image, app-extraction, and core-pipeline tests out of render/tests into their owners. Split mixed assertions into owner and render-integration cases without dropping test names. Render tests no longer link epix_core_graph. Existing camera tests in the module-mode source moved too; verification is no-modules only. All 235 tests across 11 affected targets passed. See documentation/testing.md for ownership rules. |
