...
| N29 | Render-graph startup race | OPEN | A startup-only, intermittent wgpu `invalid command buffer` abort in `wgpuQueueSubmit` (reached from `RenderGraphRunner::run` → `queue.submit` at `graph_runner.cpp:25`) occurs on heavy mesh scenes (`examples_header_mesh_batching`). It reproduces on the untouched HEAD state (~1/12 runs) as well as with the C10 depth-prep change, so it is an independent race in the parallel command-buffer submission path, not a Core2D/asset regression. The wgpu validation detail is not surfaced before the abort, so the precise failing command buffer has not been isolated. Tracked for the render-module audit; must not be conflated with mesh/core-graph parity rows. |
