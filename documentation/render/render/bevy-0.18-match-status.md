...
| N26 | Unbatchable binned-entity indices | OPEN | Bevy uses `UnbatchableBinnedEntityIndexSet::{NoEntities,Sparse,Dense}` to retain either a compact contiguous range or per-entity dynamic offsets. Epix instead exposes an optional range plus a map of prepared batches. This is both a storage and preprocessing-behavior mismatch. |
