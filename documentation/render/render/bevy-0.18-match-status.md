...
| N6 | Dynamic uniform-buffer writer | OPEN | Add the Bevy-shaped dynamic uniform writer API. The implementation may use the native queue's available upload path where the C API has no `write_buffer_with` equivalent, without exposing a divergent public API. |
| N7 | Render-graph edge-existence contract | OPEN | Replace the boolean `validate_edge(..., should_exist)` parameter with Bevy's typed `EdgeExistence` contract. |
| N8 | Sorted render-phase storage | OPEN | Replace per-view `RenderPhase<P>` components with Bevy's render-world `ViewSortedRenderPhases<I>` resource keyed by retained view entity. |
| N9 | Bind-group entry construction helpers | OPEN | Add Bevy-equivalent fixed/dynamic bind-group entries and layout-entry construction helpers. This is distinct from intentionally unsupported bindless support (R1). |
| N10 | Prepared bind-group binding resources | OPEN | Replace separate prepared buffer/texture/sampler vectors with Bevy's ordered tagged binding-resource ownership model, including unprepared binding data where required. |
