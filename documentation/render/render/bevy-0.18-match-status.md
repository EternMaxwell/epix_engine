...
| N29 | Render-graph startup race | OPEN | A startup-only, intermittent wgpu `invalid command buffer` abort in `wgpuQueueSubmit` (reached from `RenderGraphRunner::run` → `queue.submit` at `graph_runner.cpp:25`) occurs on heavy mesh scenes (`examples_header_mesh_batching`). It reproduces on untouched HEAD (~1/12 runs) and with the C10 depth-prep change at a similar rate, so it is an independent race in the parallel command-buffer submission path, not a Core2D/asset regression. Investigation so far: the abort is a Rust `nounwind` panic that cannot be caught in C++ (try/catch did not intercept it); the wgpu validation detail is not routed to the uncaptured-error callback or stderr before the abort; both Core2D main passes use `add_command_buffer_generation_task` exactly as Bevy does, so the pass-generation parallelism is not itself the cause. Next step: wrap each generation task's encoder recording in a wgpu error scope and log the validation error at pop time, then run many iterations until it trips. |
