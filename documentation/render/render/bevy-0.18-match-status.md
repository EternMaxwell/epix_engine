...
| N36 | Unit-test module ownership (FIXED) | Moved standalone camera/projection/visibility, window-reference, image, app-extraction, and core-pipeline tests out of render/tests into their owners. Split mixed assertions into owner and render-integration cases without dropping test names. Render tests no longer link epix_core_graph. Existing camera tests in the module-mode source moved too; verification is no-modules only. All 235 tests across 11 affected targets passed. See documentation/testing.md for ownership rules. Commit `07826479`. |
