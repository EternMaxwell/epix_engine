...
epix_engine provides a fixed set of schedule labels with a guaranteed order.
Startup schedules remain in `ScheduleOrder`, but their `run_once` configuration
removes their systems after the first execution. Later updates therefore pass
through empty startup schedules.
...
**Exit group** (run explicitly by the runner on shutdown; it is not in
`ScheduleOrder`):
```
...
- `run_once` removes systems after they execute; it does not remove the schedule label from `ScheduleOrder`.
