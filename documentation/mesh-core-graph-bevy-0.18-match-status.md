...
| ID | Bevy entity / behavior | Epix counterpart | Status | Notes |
| --- | --- | --- | --- | --- |
| C5 | `Core2dPlugin` required components, phase extraction, depth preparation, graph registration | `Core2dPlugin` | ❌ | Epix registers only `CameraRenderGraph{Core2d}`. Bevy also adds `DebandDither`, `Tonemapping::None`, and `ExtractComponentPlugin<Camera2d>`; only transparent phases are sorted, and core-2D depth preparation runs in `PrepareResources`. The graph/phase/depth details are tracked under C6–C12 and C17. |
| C6 | `core_2d::graph::Core2d` and all graph node labels | `Core2dGraph`, `Core2dNodes` | ❌ | Both engines pass the view entity through render-graph context rather than a configured graph input, so no input-slot mismatch exists. Epix now has the matching `Upscaling` label and no longer has Core2D-owned UI/output-blit labels, but still omits Bevy labels for MSAA writeback, wireframe, post-processing, tonemapping, FXAA/SMAA, and CAS. |
| C7 | Binned `Opaque2d`, `AlphaMask2d`, their bin keys, and batch-set key | Sorted `Opaque2D`; no `AlphaMask2D` | ❌ | The generic render module already has Bevy-style `BinnedRenderPhase`, cached membership, batch-set/bin keys, batching, and a binned-phase plugin. Core2D does not use it: opaque is sorted and alpha mask is absent. Mesh integration must supply Bevy-shaped binned keys, including whether the mesh is indexed. |
| C8 | Sorted `Transparent2d` phase item | `Transparent2D` | ❌ | Entity, main entity, cached pipeline, draw function, range, extra index, and reverse-depth sorting correspond. Epix omits Bevy's `indexed` field/accessor even though `GPUMesh` exposes it; mesh queueing therefore cannot convey the indexed indirect-draw discriminator. |
| C9 | `extract_core_2d_camera_phases` maintains opaque, alpha-mask, transparent phase resources | `extract_core2d_camera_phases` | ❌ | Epix correctly creates retained view keys, clears live sorted phases, and removes dead views. It must still prepare retained binned opaque/alpha-mask phases with `GpuPreprocessingMode::None`; Core2D UI ownership has been removed. |
...
| C11 | `MainOpaquePass2dNode` with independent opaque + alpha-mask pass work | `Node2D<Opaque2D>` | ❌ | Epix begins a correctly attached pass and applies the camera viewport, but it renders only the sorted opaque phase. It needs a dedicated opaque node that obtains and renders both binned opaque and alpha-mask phases through the existing deferred command-buffer-generation facility. |
| C12 | `MainTransparentPass2dNode` clears/render transparent pass independently | `Node2D<Transparent2D>` | ❌ | The current generic node renders the sorted transparent phase with the expected attachment, depth-store, and viewport behavior, but has no distinct node type/label or transparent-pass ownership. It should become the dedicated Bevy-shaped deferred transparent node; WebGL-only viewport-reset work is not relevant to Epix's currently forced Vulkan path. |
...
| 2026-09-01 | C5: read Bevy `core_2d/mod.rs`, `main_opaque_pass_2d_node.rs`, and `main_transparent_pass_2d_node.rs` against Epix `core2d.cpp` and `core2d.hpp`. | Core2dPlugin has a plugin-lifecycle/scheduling mismatch. No implementation started; graph, phase, depth, and upscaling sub-mismatches remain separately numbered. |
| 2026-09-01 | C6: read Bevy `core_2d::graph`, camera-driver graph invocation, and Epix graph runner/context plus `Core2dGraph`. | Corrected the tracker: the view entity is graph-context state in both engines, not a Core2D input slot. The remaining mismatch is the public graph label set and default graph chain. |
