...
All events carry a `ecs::Entity window` field identifying which window entity produced the event.
...
```cpp
struct WindowCreated { ecs::Entity window; };
```
...
```cpp
struct WindowCloseRequested { ecs::Entity window; };
```
...
```cpp
struct WindowClosed { ecs::Entity window; };
```
...
```cpp
struct WindowDestroyed { ecs::Entity window; };
```
