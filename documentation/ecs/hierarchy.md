...
void spawn_tree(Commands cmd) {
    ...
    auto parent_commands = cmd.entity(parent);
    parent_commands.spawn(Position{2, 0});
    parent_commands.spawn(Position{3, 0});
}
...
`EntityCommands::spawn()` returns commands for the newly created child. Chaining
another `.spawn()` from that return value creates a grandchild, not another
sibling.
