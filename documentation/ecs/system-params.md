...
Reads entities whose `T` component was removed or whose entity was despawned.
Every system instance has an independent reader cursor. See
[removed-components.md](./removed-components.md).
...
- `Res<T>` and `ResMut<T>` cannot coexist in the same system for the same `T`. Their access
  conflict is detected while the system initializes and is reported as an exception, not as a C++
  compilation error.
