...
- **Resources** — per-type singleton components stored on canonical resource entities
...
Resources share ordinary component storage. `resource_entity<T>()` returns the
canonical entity for a registered resource type; see [resources.md](resources.md)
for the storage invariants and clearing behavior.
...
```cpp
world.clear_entities();    // remove ordinary entities; preserve resources
world.clear_resources();   // remove resource values; preserve canonical entities
```
...
- Mutable access through `resource_mut<T>()`, `get_resource_mut<T>()`,
  `ResMut<T>`, or the canonical entity marks the component as modified.
