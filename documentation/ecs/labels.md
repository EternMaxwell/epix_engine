...
Strongly-typed identifiers for schedules and system sets. `epix.app` adds the
separate `AppLabel` wrapper for sub-applications.

## Overview

`Label` is a generic key composed of a `meta::type_index` and an optional
`uintptr_t` extra discriminator. The foundation modules expose three distinct
type-safe wrappers:
...
| `app::AppLabel`  | Identifies a sub-app (`epix.app`)        |
...
- `SystemSetLabel`, `ScheduleLabel`, and `app::AppLabel` are different types. A
  `ScheduleLabel` is not equal to a `SystemSetLabel` even when constructed from
  the same enum.
...
- Labels are small value types with equality and `std::hash` support. Store them freely.
