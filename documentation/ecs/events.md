...
- `Events<T>::update()` retires the previous old batch and ages the current
  batch. With the automatic `Last` system, an event can be read in the frame it
  was written and in the following frame; it is gone after the next update.
