...
function(epix_add_tests GROUP_NAME)
  ...
  cmake_parse_arguments(PARSE_ARGV 1 _T "" "TESTS_DIR;HEADER_DIR;MODULE_DIR" "LINK_LIBS;SHARED_DEPS")
  if(NOT _T_TESTS_DIR)
    set(_T_TESTS_DIR "tests")
  endif()
  if(NOT _T_HEADER_DIR)
    set(_T_HEADER_DIR "${_T_TESTS_DIR}/header")
  endif()
  if(NOT _T_MODULE_DIR)
    set(_T_MODULE_DIR "${_T_TESTS_DIR}/module")
  endif()

  # Header dir always built, module dir only when EPIX_CXX_MODULE=ON
  set(_dirs "${_T_HEADER_DIR}:header")
  if(EPIX_CXX_MODULE)
    list(APPEND _dirs "${_T_MODULE_DIR}:module")
  endif()

  foreach(_entry IN LISTS _dirs)
    string(REPLACE ":" ";" _parts "${_entry}")
    list(GET _parts 0 _dir_rel)
    list(GET _parts 1 _variant)
    set(_dir "${CMAKE_CURRENT_SOURCE_DIR}/${_dir_rel}")
    if(NOT EXISTS "${_dir}")
      continue()
    endif()
    ...
    foreach(_SRC IN LISTS _SRCS)
      ...
      if(_variant STREQUAL "default")
        set(_NAME "test_${GROUP_NAME}_${_REL}")
      else()
        set(_NAME "tests_${_variant}_${GROUP_NAME}_${_REL}")
      endif()
...
function(epix_add_examples GROUP_NAME)
  ...
  cmake_parse_arguments(PARSE_ARGV 1 _E "" "EXAMPLES_DIR;HEADER_DIR;MODULE_DIR;GLOB_TYPE;EXCLUDE_REGEX" "LINK_LIBS;SHARED_DEPS")
  ...
  if(NOT _E_HEADER_DIR)
    set(_E_HEADER_DIR "${_E_EXAMPLES_DIR}/header")
  endif()
  ...
  # Header dir always built, module dir only when EPIX_CXX_MODULE=ON
  set(_dirs "${_E_HEADER_DIR}:header")
  if(EPIX_CXX_MODULE)
    list(APPEND _dirs "${_E_MODULE_DIR}:module")
  endif()
  ...
  foreach(_entry IN LISTS _dirs)
    ...
    foreach(_SRC IN LISTS _SRCS)
      ...
      if(_variant STREQUAL "default")
        ...
        set(_NAME "examples_${_variant}_${GROUP_NAME}_${_REL}")
      endif()
      string(REPLACE "/" "_" _NAME "${_NAME}")
      string(REPLACE "\\" "_" _NAME "${_NAME}")
      string(REPLACE ".cpp" "" _NAME "${_NAME}")
...
function(epix_add_module MODULE_NAME)
  ...
  if (EPIX_CXX_MODULE)
    set_source_files_properties(${MODULES} PROPERTIES COMPILE_DEFINITIONS "EPIX_CXX_MODULE")
    target_sources(${MODULE_NAME}
      PUBLIC FILE_SET cxx_modules TYPE CXX_MODULES FILES ${MODULES}
    )
  endif()
  # Placeholder source for header-only targets (needed for static lib to compile)
  list(LENGTH SOURCES _src_count)
  list(LENGTH MODULES _mod_count)
  if(EPIX_CXX_MODULE)
    if(_src_count EQUAL 0 AND _mod_count EQUAL 0)
      set(_need_placeholder TRUE)
    endif()
  else()
...
function(epix_add_module_subdir MODULE_NAME SUBDIR)
  ...
  file(GLOB_RECURSE HEADERS CONFIGURE_DEPENDS "${SUBDIR}/include/**/*.hpp" "${SUBDIR}/include/**/*.h")
  ...
  # Placeholder source for header-only targets
  list(LENGTH SOURCES _src_count)
  list(LENGTH MODULES _mod_count)
  if(EPIX_CXX_MODULE)
    if(_src_count EQUAL 0 AND _mod_count EQUAL 0)
  ...
  if(EPIX_ENABLE_INSTALL)
    ...
    install(DIRECTORY ${SUBDIR}/include/ DESTINATION include)
  endif()
endfunction()
