...
struct fixed_stack {
   public:
    ...
    fixed_stack(fixed_stack&& other) noexcept
...
struct TaskSetConfig {
    ...
    static TaskSetConfig fromTask(const TaskId& id, const Task& task) {
        return TaskSetConfig{id, task, {}, {}, {}};
    }
    static TaskSetConfig fromId(const TaskId& id) {
        return TaskSetConfig{id, std::nullopt, {}, {}, {}};
    }
...
struct TaskGraph {
    ...
    void addDependency(const TaskId& from, const TaskId& to) {
        if (from == to ||
            (!task_sets.contains(from) && !task_sets.contains(to))) {
            return;
        }
    ...
    void addHierarchy(const TaskId& parent, const TaskId& child) {
        if (parent == child ||
            (!task_sets.contains(parent) && !task_sets.contains(child))) {
            return;
        }
        ...
        for (auto&& [id, set] : task_sets) {
            for (const auto& dep : set.validated_depends) {
                ...
                for (const auto& other_dep :
                     set.validated_depends |
                         std::views::filter([&](const TaskId& other) {
                             return other != dep;
                         })) {
                    if (dfs(other_dep)) {
                        // found another path, remove this dependency
                        set.validated_depends.erase(dep);
                        if (auto it = task_sets.find(dep);
                            it != task_sets.end()) {
    ...
    void rebuildCache() {
        ...
        for (auto&& [index, info] : std::views::enumerate(task_cache.sets)) {
            for (const auto& dep_index :
                 info.set->validated_depends |
                     std::views::transform([this](const TaskId& id) {
    ...
    size_t depEdgeCount() const {
        ...
        for (auto&& [id, set] : task_sets) {
            count +=
                set.validated_depends.size() + set.validated_successors.size();
        }
        return count / 2;
    }
    size_t hierarchyEdgeCount() const {
        ...
        for (auto&& [id, set] : task_sets) {
            count +=
                set.validated_parents.size() + set.validated_children.size();
        }
        return count / 2;
    }
    void execute(
        std::function<void(Task&)> task_processor = [](Task& task) { task(); },
        ...
        do {
            enter_ready();
            auto opt = finished_stack.try_pop();
            if (!opt.has_value()) {
                if (running_count == 0) {
                    break;
                }
                opt = finished_stack.pop();
            }
            size_t finished_index = *opt;
        ...
        if (remaining_count > 0) {
            print(
                "Some tasks are not executed, check for cycles in the graph.");
            ...
            for (const auto& set_info : task_cache.sets) {
                if (!set_info.entered) {
                    print(std::format(
                        ...
                        set_info.set->id,
                        set_info.depends |
                            std::views::filter([&](size_t dep_index) {
                                return !task_cache.sets[dep_index].finished;
                            }) |
                            std::views::transform([&](size_t dep_index) {
                                return task_cache.sets[dep_index].set->id;
                            }),
                        set_info.parents |
                            std::views::filter([&](size_t parent_index) {
                    ...
                    print(std::format(
                        "Set {} is entered but not finished. "
                        "children: {} unfinished.",
                        set_info.set->id,
                        set_info.children |
                            std::views::filter([&](size_t child_index) {
...
struct TaskGraph {
    ...
    TaskGraph(const task_graph::TaskGraph& set_graph)
        : thread_pool(set_graph.thread_pool.threadCount()) {
        for (const auto& [id, set] :
             set_graph.task_sets | std::views::filter([](const auto& pair) {
                 return pair.second.task.has_value();
             })) {
            TaskConfig config(id, set.task.value());
            config.dependencies =
                set.depends | std::views::filter([&](const TaskId& dep) {
                    return set_graph.task_sets.contains(dep) &&
                           set_graph.task_sets.at(dep).task.has_value();
                }) |
                std::ranges::to<std::unordered_set<TaskId>>();
            config.successors =
                set.successors | std::views::filter([&](const TaskId& succ) {
                    return set_graph.task_sets.contains(succ) &&
                           set_graph.task_sets.at(succ).task.has_value();
                }) |
                std::ranges::to<std::unordered_set<TaskId>>();
            addTask(config);
        }
        ...
        for (const auto& [id, _] : set_graph.task_sets) {
            std::unordered_set<TaskId> visited;
            std::function<void(const TaskId&)> dfs = [&](const TaskId&
                                                             current) {
        ...
        for (const auto& [id, children] : all_child_ids) {
            for (const auto& dependencies :
                 set_graph.task_sets.at(id).depends |
                     std::views::filter([&](const TaskId& dep) {
                         return set_graph.task_sets.contains(dep);
                     })) {
            ...
            for (const auto& successors :
                 set_graph.task_sets.at(id).successors |
                     std::views::filter([&](const TaskId& succ) {
                         return set_graph.task_sets.contains(succ);
                     })) {
    ...
    size_t edgeCount() const {
        ...
        for (const auto& [id, node] : tasks) {
            count += node.validated_dependencies.size() +
                     node.validated_successors.size();
        }
                ...
                std::function<bool(const TaskId&)> dfs =
                    [&](const TaskId& current) {
                        ...
                        if (auto it = tasks.find(current); it != tasks.end()) {
                            ...
                            for (const auto& next :
                                 current_node.validated_dependencies) {
                                if (dfs(next)) return true;  // continue DFS
                            }
                        }
                        return false;  // no path found
...
task_graph::TaskGraph* random_generate(size_t count,
                                       double task_prob,
                                       double dep_prob,
                                       double hier_prob) {
    ...
    std::vector<std::pair<size_t, size_t>> remainging_slots =
        std::views::cartesian_product(std::views::iota(0u, count),
    ...
    std::sort(remainging_slots.begin(), remainging_slots.end(),
    ...
    for (auto&& [i, j] : remainging_slots) {
        if (prob_dist(rng) < task_prob) {
            taskSetConfigs[i][j] = TaskSetConfig::fromTask(
                std::format("task{}-{}.{}", taskCount, i, j),
                make_task(std::format("task{}", taskCount), dur_dist(rng)));
            taskCount++;
        } else {
            taskSetConfigs[i][j] = TaskSetConfig::fromId(
                std::format("set{}-{}.{}", setCount, i, j));
            setCount++;
        }
    }
    ...
    for (auto&& [x, y] :
        ...
        for (auto&& [ox, oy] :
             std::views::cartesian_product(std::views::iota(x + 1, count),
                                           std::views::iota(0u, y)) |
                 std::views::filter([&](const std::pair<size_t, size_t>& p) {
                     return taskSetConfigs[p.first][p.second].has_value();
                 })) {
            if (prob_dist(rng) < hier_prob) {
                taskSetConfigs[x][y]->contains(
                    taskSetConfigs[ox][oy]->id);  // add hierarchy
            }
        }
    ...
    std::function<void(const TaskId&)> dfs = [&](const TaskId& current) {
        ...
        for (const auto& child : taskGraph->task_sets[current].children) {
            ...
            all_child_ids[current].insert(
                all_child_ids[child].begin(),
    ...
    std::function<bool(const TaskId&, const TaskId&)> can_add_dependency =
        [&](const TaskId& from, const TaskId& to) {
            if (from == to) return false;  // no self-dependency
            if (all_child_ids[from].contains(to))
                return false;  // already a child
            if (all_child_ids[to].contains(from))
            ...
            std::function<bool(const TaskId&)> dfs =
                [&](const TaskId& current) {
                    ...
                    for (const auto& succ :
                         taskGraph->task_sets[current].successors) {
    ...
    for (auto&& [x, y] :
        ...
        for (auto&& [ox, oy] :
             std::views::cartesian_product(std::views::iota(x + 1, count),
                                           std::views::iota(y + 1, count)) |
                 std::views::filter([&](const std::pair<size_t, size_t>& p) {
                     return taskSetConfigs[p.first][p.second].has_value();
                 })) {
            if (prob_dist(rng) < dep_prob &&
                can_add_dependency(taskSetConfigs[ox][oy]->id,
                                   taskSetConfigs[x][y]->id)) {
                taskGraph->addDependency(taskSetConfigs[ox][oy]->id,
                                         taskSetConfigs[x][y]->id);
            }
        }
    }
...
int main() {
    ...
    std::unique_ptr<task_graph::TaskGraph> randomGraph(
        random_generate(1000, 0.9, 0.2, 0.005));
    ...
    randomGraph
        ->simplify_validated();  // optional, to remove unnecessary dependencies
    ...
    print(std::format(
        "Set Graph: Set count: {}, Task count: {}, Dep edge count: {}, "
        "Hierarchy edge count: {}",
        randomGraph->setCount(), randomGraph->taskCount(),
        randomGraph->depEdgeCount(), randomGraph->hierarchyEdgeCount()));
    randomGraph->execute();  // multi-threaded execution

    std::unique_ptr<old_graph::TaskGraph> oldGraph(
        new old_graph::TaskGraph(*randomGraph));
    print("Old Graph");
    oldGraph->validate();
    print(std::format("Old Graph: Task count: {}, Edge count: {}",
                      oldGraph->taskCount(), oldGraph->edgeCount()));
    oldGraph->rebuildCache();
    oldGraph->execute();  // multi-threaded execution
    oldGraph->simplify_validated();
    print("Old Graph Simplified");
    print(std::format("Old Graph Simplified: Task count: {}, Edge count: {}",
                      oldGraph->taskCount(), oldGraph->edgeCount()));
    oldGraph->rebuildCache();
    oldGraph->execute();  // multi-threaded execution
    return 0;
