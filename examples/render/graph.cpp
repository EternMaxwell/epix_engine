...
struct DriverNode : Node {
    void run(GraphContext& graph_ctx, RenderContext& render_ctx, const app::World& world) override {
...
struct OutputNode : Node {
    ...
    std::vector<SlotInfo> outputs() override { return {{"output_slot", SlotType::Entity}}; }
};
...
struct InputNode : Node {
    void run(GraphContext& graph_ctx, RenderContext& render_ctx, const app::World& world) override {
        ...
        if (pvalue) {
            spdlog::info("Input node received entity with id: {}", pvalue->entity().value().index());
        } else {
            spdlog::warn("Input node did not receive an entity.");
        }
    }
    std::vector<SlotInfo> inputs() override { return {{"input_slot", SlotType::Entity}}; }
};
