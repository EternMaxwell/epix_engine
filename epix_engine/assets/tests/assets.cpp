...
void test_1() {
    ...
    if (auto&& opt = assets.get(*handle1)) {
        ...
        std::cerr << "Test fail: Handle1 is invalid after creation and handle assign." << std::endl;
        return;
    }
...
void test_2() {
    ...
    if (!handle2_opt) {
        std::cerr << "Test fail: Cannot get strong handle from handle1" << std::endl;
        return;
    }
...
void test_3() {
    ...
    epix::assets::AssetIndex index1 = std::get<epix::assets::AssetIndex>(handle1->id());
    ...
    epix::assets::AssetIndex index2 = std::get<epix::assets::AssetIndex>(handle2.id());
    ...
    if (index1.generation() + 1 != index2.generation()) {
        std::cerr << "Test fail: Index2 generation is not equal to Index1 "
...
void test_4() {
    ...
    std::cout << "===== Test 4: Force removal and replacement =====" << std::endl;
    std::cout << description << std::endl;
    std::this_thread::sleep_for(std::chrono::milliseconds(500));
    ...
    // Check if the handle is valid
    if (auto&& opt = assets.get(handle1)) {
        std::cerr << "Test fail: Handle should be invalid after force removal" << std::endl;
        return;
    }
    ...
    if (auto&& opt = assets.try_get(handle1)) {
        ...
        if (str != "Hello Assets2!") {
            std::cerr << "Test fail: Insert value is not the expected value." << std::endl;
            return;
        }
    } else {
...
void test_5() {
    ...
    std::cout << "===== Test 6: Reserving handles outside Asset<T> =====" << std::endl;
    std::cout << description << std::endl;
    std::this_thread::sleep_for(std::chrono::milliseconds(500));
    ...
    if (!res) {
        std::cerr << "Test fail: Unable to insert new value at the index: "
