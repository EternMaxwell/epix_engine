...
EPIX_EXPORT struct AssetServer;
struct AssetInfos;
EPIX_EXPORT struct ProcessContext;
EPIX_EXPORT struct LoadContext;
EPIX_EXPORT template <Asset A>
struct LoadedAsset;
EPIX_EXPORT template <Asset A>
struct SavedAsset;
EPIX_EXPORT template <Asset A>
...
EPIX_EXPORT struct ErasedLoadedAsset {
   private:
...
struct LoadedAsset {
   ...
   public:
    ...
    /** @brief Get a range over all label strings. */
    auto labels() const {
        return std::views::transform(labeled_assets, [](const auto& pair) -> const std::string& { return pair.first; });
    }
};
...
EPIX_EXPORT namespace load_error {
...
EPIX_EXPORT using AssetLoadError = std::variant<load_error::RequestHandleMismatch,
...
EPIX_EXPORT enum LoadStateOK { NotLoaded, Loading, Loaded };
...
EPIX_EXPORT using LoadState = std::variant<LoadStateOK, std::shared_ptr<AssetLoadError>>;

/** @brief Load state of an asset's direct dependencies. */
EPIX_EXPORT struct DependencyLoadState : std::variant<LoadStateOK, std::shared_ptr<AssetLoadError>> {
...
EPIX_EXPORT struct RecursiveDependencyLoadState : std::variant<LoadStateOK, std::shared_ptr<AssetLoadError>> {
    using base = std::variant<LoadStateOK, std::shared_ptr<AssetLoadError>>;
    using base::base;
...
EPIX_EXPORT namespace wait_for_asset_error {
    struct NotLoaded {};
    struct Failed {
...
EPIX_EXPORT using WaitForAssetError =
...
EPIX_EXPORT std::variant<std::string, std::exception_ptr> format_asset_load_error(const AssetLoadError& error);
...
EPIX_EXPORT void log_asset_load_error(const AssetLoadError& error, const AssetPath& path);
...
EPIX_EXPORT struct NestedLoader;
