...
EPIX_EXPORT struct NotFoundError {
    std::error_code code;
    std::filesystem::path path;
};

EPIX_EXPORT struct IoError {
    std::error_code code;
    std::filesystem::path path;
};

EPIX_EXPORT struct ExceptionError {
    std::exception_ptr cause;
    std::filesystem::path path;
};

EPIX_EXPORT using DirectoryError = std::variant<NotFoundError, IoError, ExceptionError>;

EPIX_EXPORT enum class DirEventType { FileAdded, FileModified, FileRemoved, DirAdded, DirRemoved, Moved };

EPIX_EXPORT struct DirEvent {
    DirEventType type;
