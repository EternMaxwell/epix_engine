...
EPIX_EXPORT struct FileAssetReader : public AssetReader {
   private:
...
EPIX_EXPORT struct FileAssetWriter : public AssetWriter {
   private:
    std::filesystem::path m_root;

   public:
    FileAssetWriter(std::filesystem::path root) : m_root(std::filesystem::absolute(std::move(root))) {}
    STDEXEC::task<std::expected<std::unique_ptr<Writer>, AssetWriterError>> write(
        const std::filesystem::path& path) const override;
    ...
    STDEXEC::task<std::expected<void, AssetWriterError>> create_directory(
        const std::filesystem::path& path) const override;
    STDEXEC::task<std::expected<void, AssetWriterError>> remove_directory(
        const std::filesystem::path& path) const override;
    STDEXEC::task<std::expected<void, AssetWriterError>> clear_directory(
        const std::filesystem::path& path) const override;
};
