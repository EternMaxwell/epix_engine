...
EPIX_EXPORT namespace reader_errors {
    struct NotFound {
...
EPIX_EXPORT using AssetReaderError =
...
EPIX_EXPORT namespace writer_errors {
    struct IoError {
        std::error_code code;
    };
...
EPIX_EXPORT namespace source_events {
    struct AddedAsset {
        std::filesystem::path path;
    };
...
EPIX_EXPORT using AssetSourceEvent = std::variant<source_events::AddedAsset,
...
EPIX_EXPORT struct AssetWatcher {
...
EPIX_EXPORT struct VecReader : Reader {
   private:
...
EPIX_EXPORT struct VecWriter : Writer {
   private:
