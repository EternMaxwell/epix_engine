...
EPIX_EXPORT enum class ProcessorState {
    Initializing,
    Processing,
    Finished,
};
...
EPIX_EXPORT namespace set_transaction_log_factory_errors {
    struct AlreadyInUse {};
}  // namespace set_transaction_log_factory_errors

EPIX_EXPORT using SetTransactionLogFactoryError = std::variant<set_transaction_log_factory_errors::AlreadyInUse>;

/** @brief Shared data for the AssetProcessor, accessible across threads.
 *  Matches bevy_asset's AssetProcessorData. */
EPIX_EXPORT struct AssetProcessorData {
   private:
