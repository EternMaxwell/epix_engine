...
using AssetMetaCheck = std::variant<asset_meta_check::Always, asset_meta_check::Never, asset_meta_check::Paths>;

/** @brief Controls how unapproved asset paths are handled.
 *  Matches bevy_asset's UnapprovedPathMode. */
enum class UnapprovedPathMode {
...
template <typename LoaderSettings, typename ProcessSettings>
struct AssetAction {
    struct Load {
...
using MetaTransform = std::function<void(AssetMetaDyn&)>;
...
template <typename LoaderSettings, typename ProcessSettings = EmptySettings>
...
inline std::expected<AssetMetaMinimal, std::errc> deserialize_meta_minimal(const std::vector<std::byte>& bytes) {
...
template <typename LS, typename PS>
