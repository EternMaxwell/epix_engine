...
EPIX_EXPORT inline constexpr std::string_view META_FORMAT_VERSION = "2.0";

/** @brief Controls when and how asset metadata files are checked.
 *  Matches bevy_asset's AssetMetaCheck. */
EPIX_EXPORT namespace asset_meta_check {
...
EPIX_EXPORT using AssetMetaCheck =
    std::variant<asset_meta_check::Always, asset_meta_check::Never, asset_meta_check::Paths>;

/** @brief Controls how unapproved asset paths are handled.
 *  Matches bevy_asset's UnapprovedPathMode. */
EPIX_EXPORT enum class UnapprovedPathMode {
...
EPIX_EXPORT using AssetHash = std::array<uint8_t, 32>;

/** @brief Information about a processed asset's dependency on another asset. */
EPIX_EXPORT struct ProcessDependencyInfo {
   private:
...
EPIX_EXPORT struct ProcessedInfo {
   private:
    friend zpp::bits::access;
...
EPIX_EXPORT enum class AssetActionType {
...
EPIX_EXPORT template <typename LoaderSettings, typename ProcessSettings>
struct AssetAction {
    struct Load {
...
EPIX_EXPORT struct EmptySettings {};
...
EPIX_EXPORT struct AssetMetaDyn {
...
EPIX_EXPORT using MetaTransform = std::function<void(AssetMetaDyn&)>;
...
EPIX_EXPORT template <typename LoaderSettings, typename ProcessSettings = EmptySettings>
...
EPIX_EXPORT struct AssetActionMinimal {
...
EPIX_EXPORT struct AssetMetaMinimal {
...
EPIX_EXPORT std::expected<std::vector<std::byte>, std::errc> serialize_meta_minimal(const AssetMetaMinimal& minimal);

/** @brief Deserialize an AssetMetaMinimal from a byte span using zpp::bits. */
EPIX_EXPORT std::expected<AssetMetaMinimal, std::errc> deserialize_meta_minimal(std::span<const std::byte> bytes);

/** @brief Overload accepting a std::vector<std::byte>. */
EPIX_EXPORT inline std::expected<AssetMetaMinimal, std::errc> deserialize_meta_minimal(
...
EPIX_EXPORT std::expected<std::optional<ProcessedInfo>, std::errc> deserialize_processed_info(
    std::span<const std::byte> bytes);

/** @brief Overload accepting std::vector<std::byte>. */
EPIX_EXPORT inline std::expected<std::optional<ProcessedInfo>, std::errc> deserialize_processed_info(
    const std::vector<std::byte>& bytes) {
