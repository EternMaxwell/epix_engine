...
EPIX_EXPORT using AssetNotPresent = std::variant<AssetIndex, uuids::uuid>;

/** @brief Sum type of all possible asset access errors. */
EPIX_EXPORT using AssetError = std::variant<IndexOutOfBound, SlotEmpty, GenMismatch, AssetNotPresent>;
...
EPIX_EXPORT struct LoadedFolder {
...
EPIX_EXPORT struct LoadedUntypedAsset {
...
EPIX_EXPORT struct UntypedAssetLoadFailedEvent {
    /** @brief The id of the asset that failed to load. */
    UntypedAssetId id;
    /** @brief The path that was attempted. */
    AssetPath path;
