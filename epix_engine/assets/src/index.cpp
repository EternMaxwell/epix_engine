...
AssetIndexAllocator::AssetIndexAllocator() {
    std::tie(m_free_indices_sender, m_free_indices_receiver) = async_channel::unbounded<AssetIndex>();
    std::tie(m_reserved_sender, m_reserved)                  = async_channel::unbounded<AssetIndex>();
}
AssetIndex AssetIndexAllocator::reserve() const {
    if (auto index = m_free_indices_receiver.try_recv()) {
        spdlog::trace("[assets] AssetIndexAllocator::reserve: recycled index={}/gen={}.", index->index(),
                      index->generation() + 1u);
        m_reserved_sender.try_send(AssetIndex(index->index(), index->generation() + 1u));
        ...
        m_reserved_sender.try_send(AssetIndex(i, 0));
        return AssetIndex(i, 0);
    }
}
async_channel::Receiver<AssetIndex> AssetIndexAllocator::reserved_receiver() const noexcept { return m_reserved; }
