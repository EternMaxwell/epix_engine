...
std::optional<UntypedHandle> AssetInfos::get_or_create_handle_internal(
    const std::filesystem::path& path, const std::optional<epix::meta::type_index>& type, bool force_new) {
    auto&& ids          = path_to_ids[path];
    auto asset_type_opt = type.or_else([&]() -> std::optional<epix::meta::type_index> {
...
const ErasedAssetLoader* AssetLoaders::get_by_type(const epix::meta::type_index& type) const {
...
std::vector<const ErasedAssetLoader*> AssetLoaders::get_multi_by_type(
    const epix::meta::type_index& type) const {  // get all loaders of a specific type
...
AssetServer::AssetServer() : asset_infos(), asset_loaders() {
    std::tie(event_sender, event_receiver) = epix::utils::async::make_channel<InternalAssetEvent>();
}
            ...
            info.waiter = std::async(std::launch::async, [this, id, context, loaders = std::move(loaders)]() mutable {
                ...
                for (auto& loader :
                    ...
                    spdlog::trace(
                        ...
                        context.path.string(), id.type.name(), loader->loader_type().name());
                    try {
                        auto asset = loader->load(context.path, context);
                        if (asset.value) {
                            event_sender.send(AssetLoadedEvent{id, std::move(asset)});
