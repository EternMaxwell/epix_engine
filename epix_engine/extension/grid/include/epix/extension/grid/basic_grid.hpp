...
struct packed_grid {
   ...
   public:
     ...
     */
    std::size_t dimension(std::size_t index) const noexcept { return m_dimensions[index]; }
    /**
     * @brief Get the total number of cells in the grid.
     * @return Total number of cells.
     */
    std::size_t count() const noexcept { return m_cells.size(); }
     ...
     */
    std::expected<std::reference_wrapper<T>, grid_error> get(const std::array<std::uint32_t, Dim>& pos) noexcept {
        return offset(pos).transform([this](std::size_t index) { return std::ref(m_cells[index]); });
    }
    T& get_unsafe(const std::array<std::uint32_t, Dim>& pos) noexcept { return m_cells[offset_unsafe(pos)]; }
    /**
...
struct dense_grid {
   ...
   public:
     ...
     * @return Array of sizes along each axis.
     */
    std::array<std::uint32_t, Dim> dimensions() const noexcept { return m_index_grid.dimensions(); }
    /**
     * @brief Get the size along a single axis.
     * @param index Axis index.
     * @return Size along the given axis.
     */
    std::size_t dimension(std::size_t index) const noexcept { return m_index_grid.dimension(index); }
    /**
     * @brief Get the total number of occupied cells in the grid.
     * @return Number of occupied cells.
     */
    std::size_t count() const noexcept { return m_data.size(); }
    /** @brief Clear all cells in the grid. */
    void clear() {
        m_data.clear();
        m_positions.clear();
     ...
     */
    std::expected<std::reference_wrapper<T>, grid_error> get(const std::array<std::uint32_t, Dim>& pos) noexcept;
    T& get_unsafe(const std::array<std::uint32_t, Dim>& pos) noexcept;
    /**
     * @brief Get a const reference to the cell at the given position.
     * @param pos Position in the grid.
...
struct sparse_grid {
   ...
   public:
     ...
     */
    std::size_t count() const noexcept { return m_data.size() - m_recycled_indices.size(); }
    /** @brief Clear all cells in the grid. */
    void clear() {
        m_data.clear();
        m_positions.clear();
 ...
 */
template <std::size_t Dim, typename T, std::size_t ChildCount = 2>
    requires std::movable<T>
struct tree_extendible_grid {
   ...
   public:
     ...
     * @return Array of sizes along each axis.
     */
    std::array<std::uint32_t, Dim> dimensions() const noexcept;
    /**
     * @brief Get the size along a single axis.
     * @param index Axis index.
     * @return Size along the given axis.
     */
    std::size_t dimension(std::size_t index) const noexcept;
    /**
...
struct tree_grid {
   ...
   public:
     ...
     */
    std::expected<std::reference_wrapper<T>, grid_error> get(const std::array<std::uint32_t, Dim>& pos) noexcept;
    T& get_unsafe(const std::array<std::uint32_t, Dim>& pos);
    /** @brief Get a const reference to the cell at the given position.
     *  @param pos Unsigned position to query.
     *  @return Const reference to the cell, or grid_error::EmptyCell / grid_error::OutOfBounds.
     */
...
template <std::size_t Dim, typename T>
    requires std::movable<T>
T& dense_grid<Dim, T>::get_unsafe(const std::array<std::uint32_t, Dim>& pos) noexcept {
...
std::expected<void, grid_error> dense_grid<Dim, T>::remove(const std::array<std::uint32_t, Dim>& pos) {
    ...
    if (index != last_index) {
        ...
        (void)m_index_grid.set(m_positions[index], index);
    }
    m_data.pop_back();
...
template <std::size_t Dim, typename T>
    requires std::movable<T>
T& sparse_grid<Dim, T>::get_unsafe(const std::array<std::uint32_t, Dim>& pos) noexcept {
    return m_data[m_index_grid.get_unsafe(pos)];
}
template <std::size_t Dim, typename T>
    requires std::movable<T>
...
template <std::size_t Dim, typename T>
    requires std::movable<T>
const T& dense_extendible_grid<Dim, T>::get_unsafe(const std::array<std::int32_t, Dim>& pos) const noexcept {
    return m_data[m_index_grid.get_unsafe(relative_pos_unsafe(pos))];
}
template <std::size_t Dim, typename T>
    requires std::movable<T>
...
template <std::size_t Dim, typename T, std::size_t ChildCount>
    requires std::movable<T>
bool tree_extendible_grid<Dim, T, ChildCount>::contains(const std::array<std::int32_t, Dim>& pos) const noexcept {
    return find_index(pos).has_value();
}
template <std::size_t Dim, typename T, std::size_t ChildCount>
    requires std::movable<T>
std::uint32_t tree_extendible_grid<Dim, T, ChildCount>::coverage() const noexcept {
    return axis_coverage();
}
template <std::size_t Dim, typename T, std::size_t ChildCount>
    requires std::movable<T>
std::array<std::uint32_t, Dim> tree_extendible_grid<Dim, T, ChildCount>::dimensions() const noexcept {
    return dimensions_impl();
}
template <std::size_t Dim, typename T, std::size_t ChildCount>
    requires std::movable<T>
std::size_t tree_extendible_grid<Dim, T, ChildCount>::dimension(std::size_t index) const noexcept {
    return dimensions_impl()[index];
}
template <std::size_t Dim, typename T, std::size_t ChildCount>
    requires std::movable<T>
std::size_t tree_extendible_grid<Dim, T, ChildCount>::count() const noexcept {
    return m_data.size();
}
template <std::size_t Dim, typename T, std::size_t ChildCount>
    requires std::movable<T>
...
template <std::size_t Dim, typename T, std::size_t ChildCount>
    requires std::movable<T>
std::uint32_t tree_grid<Dim, T, ChildCount>::coverage() const noexcept {
    return axis_coverage();
}
template <std::size_t Dim, typename T, std::size_t ChildCount>
    requires std::movable<T>
std::array<std::uint32_t, Dim> tree_grid<Dim, T, ChildCount>::dimensions() const noexcept {
    return dimensions_impl();
}
template <std::size_t Dim, typename T, std::size_t ChildCount>
    requires std::movable<T>
std::size_t tree_grid<Dim, T, ChildCount>::dimension(std::size_t index) const noexcept {
    return dimensions_impl()[index];
}
template <std::size_t Dim, typename T, std::size_t ChildCount>
    requires std::movable<T>
std::size_t tree_grid<Dim, T, ChildCount>::count() const noexcept {
    return m_data.size();
}
template <std::size_t Dim, typename T, std::size_t ChildCount>
    requires std::movable<T>
...
template <std::size_t Dim, typename T, std::size_t ChildCount>
    requires std::movable<T>
T& tree_grid<Dim, T, ChildCount>::get_unsafe(const std::array<std::uint32_t, Dim>& pos) {
    return m_data[find_index_unsafe(pos)];
}
template <std::size_t Dim, typename T, std::size_t ChildCount>
    requires std::movable<T>
