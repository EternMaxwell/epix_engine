...
struct Time<Real> : private Time<> {
    ...
    explicit Time(std::chrono::steady_clock::time_point startup) noexcept : m_context{.startup = startup} {}

    /** @brief Update using the current steady_clock::now(). */
    void update() noexcept { update_with_instant(std::chrono::steady_clock::now()); }

    /** @brief Update by advancing a fixed duration from the last update point. */
    void update_with_duration(std::chrono::nanoseconds duration) noexcept {
    ...
    void update_with_instant(std::chrono::steady_clock::time_point instant) noexcept {
    ...
    std::chrono::steady_clock::time_point startup() const noexcept { return m_context.startup; }
    ...
    std::optional<std::chrono::steady_clock::time_point> first_update() const noexcept {
        return m_context.first_update;
    }

    /** @brief Get the time point of the most recent update, if any. */
    std::optional<std::chrono::steady_clock::time_point> last_update() const noexcept { return m_context.last_update; }

    /** @brief Get const reference to the Real context. */
    const Real& context() const noexcept { return m_context; }
    /** @brief Get mutable reference to the Real context. */
    Real& context_mut() noexcept { return m_context; }
