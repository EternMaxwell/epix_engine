...
EPIX_API const std::optional<epix::Entity>& Focus::operator*() const { return focus; }
...
EPIX_API void WindowPlugin::build(epix::App& app) {
    ...
    if (primary_window) {
        ...
        if (auto focus = app.get_resource<Focus>()) {
            focus->focus = window;
            app.add_systems(PreUpdate, into([](ResMut<Focus> focus, Local<Focus> last,
                                               Query<Item<Entity, Mut<Window>>> query, Local<int> frame) {
                                           if (focus->focus != last->focus) {  // force the focused window to be the
                                                                               // one that is focused
                                               if (focus->focus) {
...
EPIX_API void epix::window::print_events(EventReader<events::WindowResized> resized,
