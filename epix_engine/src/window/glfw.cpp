...
EPIX_API void GLFWPlugin::update_size(Query<Item<Entity, Mut<window::Window>>>& windows,
...
EPIX_API void GLFWPlugin::update_pos(Commands commands,
    ...
    /// final pos.
    entt::dense_map<Entity, std::pair<int, int>> final_positions;
    int monitor_count = 0;
    auto monitors     = glfwGetMonitors(&monitor_count);
    ...
    calculate = [&](Entity id) -> std::pair<int, int> {
        ...
        auto&& [_, desc, parent]                               = windows.get(id);
        std::optional<std::tuple<GLFWwindow*, Window&>> stored = std::nullopt;
        if (auto it = glfw_windows->find(id); it != glfw_windows->end()) {
            stored.emplace(it->second.first, it->second.second);
        }
...
EPIX_API void GLFWPlugin::create_windows(Commands commands,
...
EPIX_API void GLFWPlugin::update_window_states(Query<Item<Entity, Mut<window::Window>>>& windows,
                                               ResMut<GLFWwindows>& glfw_windows,
    ...
    for (auto&& [id, desc] : windows.iter()) {
        ...
        if (cached.cursor_icon != desc.cursor_icon) {
            std::visit(epix::util::visitor{[&](const StandardCursor& icon) {
...
EPIX_API void GLFWPlugin::toggle_window_mode(Query<Item<Entity, Mut<window::Window>>>& windows,
                                             ResMut<GLFWwindows>& glfw_windows,
...
EPIX_API void GLFWPlugin::send_cached_events(
    ...
    for (auto&& [id, pair] : *glfw_windows) {
        ...
        while (auto new_size = user_data->resized.try_pop()) {
            // send out
            window_resized.write(window::events::WindowResized{id, new_size->width, new_size->height});
        }
        {
            if (glfwWindowShouldClose(window)) {
                window_close_requested.write(window::events::WindowCloseRequested{id});
            }
        }
        while (auto key_input = user_data->key_input.try_pop()) {
            ...
            if (key_input_event)
                key_input_event->write(
                    input::events::KeyInput{map_glfw_key_to_input(key), scancode, pressed, repeat, id});
        }
        while (auto cursor_pos = user_data->cursor_pos.try_pop()) {
            // send out
            auto [new_x, new_y] = *cursor_pos;
            auto [old_x, old_y] = cached.cursor_pos;
            cursor_moved.write(window::events::CursorMoved{id, {new_x, new_y}, {new_x - old_x, new_y - old_y}});
            if (mouse_move_input) mouse_move_input->write(input::events::MouseMove{{new_x - old_x, new_y - old_y}});
        }
        ...
        while (auto mouse_button = user_data->mouse_button.try_pop()) {
            ...
            if (mouse_button_input)
                mouse_button_input->write(
                    input::events::MouseButtonInput{map_glfw_mouse_button_to_input(button), pressed, id});
        }
        while (auto scroll = user_data->scroll.try_pop()) {
            // send out
            auto&& [xoffset, yoffset] = *scroll;
            if (scroll_input) scroll_input->write(input::events::MouseScroll{xoffset, yoffset, id});
        }
        ...
        while (auto character = user_data->received_character.try_pop()) {
            ...
            received_character.write(window::events::ReceivedCharacter{id, codepoint});
        }
...
EPIX_API void GLFWPlugin::destroy_windows(Query<Item<Entity, window::Window>> windows,
                                          ResMut<GLFWwindows> glfw_windows,
