...
EPIX_API RunStateData::RunStateData(async::RwLock<World>::WriteGuard&& world,
                                    Executors& executors)
    : world(std::move(world)), executors(&executors) {
    m_apply_commands = IntoSystem::into_system(
        [](World& world) { world.command_queue().apply(world); });
    m_apply_commands->initialize(*this->world);
}
EPIX_API RunStateData::~RunStateData() { wait(); }

EPIX_API void RunStateData::apply_commands() {
    run_system(
        m_apply_commands.get(),
        RunState::RunSystemConfig{.on_finish = []() {},
                                  .on_start  = []() {},
                                  .executor  = ExecutorType::SingleThread});
}
EPIX_API bool RunStateData::wait() {
...
EPIX_API RunState::RunState(async::RwLock<World>::WriteGuard&& world,
                            Executors& executors)
    : data(new RunStateData(std::move(world), executors)) {}
EPIX_API void RunState::apply_commands() { data->apply_commands(); }
EPIX_API bool RunState::wait() { return data->wait(); }
...
EPIX_API Executors::executor_t* Executors::get_pool(
    const ExecutorLabel& label) noexcept {
...
EPIX_API void Executors::add_pool(const ExecutorLabel& label,
