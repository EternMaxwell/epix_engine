...
EPIX_API RenderPipelineDesc& RenderPipelineDesc::setVertexShader(
    ShaderInfo shader) {
    vertexShader = std::move(shader);
    return *this;
}
EPIX_API RenderPipelineDesc& RenderPipelineDesc::setFragmentShader(
    ShaderInfo shader) {
    fragmentShader = std::move(shader);
    return *this;
}
EPIX_API RenderPipelineDesc& RenderPipelineDesc::setGeometryShader(
    ShaderInfo shader) {
    geometryShader = std::move(shader);
    return *this;
}
EPIX_API RenderPipelineDesc& RenderPipelineDesc::setTessellationControlShader(
    ShaderInfo shader) {
    tessellationControlShader = std::move(shader);
    return *this;
}
EPIX_API RenderPipelineDesc&
RenderPipelineDesc::setTessellationEvaluationShader(ShaderInfo shader) {
    tessellationEvaluationShader = std::move(shader);
    return *this;
}
EPIX_API RenderPipelineDesc& RenderPipelineDesc::addBindingLayout(
    const nvrhi::BindingLayoutDesc& layout) {
    bindingLayoutDescs.push_back(layout);
    return *this;
}

EPIX_API ComputePipelineDesc& ComputePipelineDesc::setComputeShader(
    ShaderInfo shader) {
    computeShader = std::move(shader);
    return *this;
}
EPIX_API ComputePipelineDesc& ComputePipelineDesc::addBindingLayout(
    const nvrhi::BindingLayoutDesc& layout) {
    bindingLayoutDescs.push_back(layout);
    return *this;
}

EPIX_API PipelineServer::RenderPipelineCache::RenderPipelineCache(
...
EPIX_API PipelineServer::RenderPipelineCache&
PipelineServer::RenderPipelineCache::operator=(RenderPipelineCache&& other) {
    if (this != &other) {
        device               = std::move(other.device);
        desc                 = std::move(other.desc);
        _id                  = std::move(other._id);
...
EPIX_API RenderPipelineId PipelineServer::RenderPipelineCache::id() const {
    return _id;
}
EPIX_API size_t PipelineServer::RenderPipelineCache::specialize(
...
EPIX_API nvrhi::GraphicsPipelineHandle PipelineServer::RenderPipelineCache::get(
    size_t index) const {
    std::lock_guard lock(_mutex);
...
EPIX_API std::tuple<size_t, nvrhi::GraphicsPipelineHandle>
PipelineServer::RenderPipelineCache::get(
    nvrhi::FramebufferHandle framebuffer) const {
    size_t index = specialize(framebuffer);
    return {index, get(index)};
}

EPIX_API PipelineServer::PipelineServer(nvrhi::DeviceHandle device)
    : device(device) {}
...
PipelineServer::queue_render_pipeline(const RenderPipelineDesc& desc) {
...
PipelineServer::queue_compute_pipeline(const ComputePipelineDesc& desc) {
    ComputePipelineId id{computePipelines.size()};
    computePipelines.emplace_back(desc);
    queuedPipelines.emplace_back(id);
    return id;
}
EPIX_API std::optional<RenderPipeline> PipelineServer::get_render_pipeline(
...
EPIX_API std::optional<ComputePipeline> PipelineServer::get_compute_pipeline(
...
EPIX_API void PipelineServer::process_queued(
    ResMut<PipelineServer> server, Res<assets::RenderAssets<Shader>> shaders) {
                ...
                [&](const RenderPipelineId& id) {
                    ...
                    if (!renderDesc.PS) {
                        if (!renderDesc.fragmentShader.shader) {
                            ...
                            server->renderPipelines[id] = std::make_pair(
                                renderDesc,
                                PipelineError{"No fragment shader provided"});
                            return;
                        } else {
