...
EPIX_API GraphContext::GraphContext(
    ...
    : m_graph(graph),
      ...
      m_inputs(inputs),
      m_outputs(outputs) {}
EPIX_API const std::vector<SlotValue>& GraphContext::inputs() const {
    return m_inputs;
}
EPIX_API const SlotInfos& GraphContext::input_info() const {
    return m_node_state.inputs;
}
EPIX_API const SlotInfos& GraphContext::output_info() const {
    return m_node_state.outputs;
}
EPIX_API const SlotValue* GraphContext::get_input(
...
EPIX_API std::optional<epix::app::Entity> GraphContext::get_input_entity(
    const SlotLabel& label) const {
...
EPIX_API std::optional<nvrhi::BufferHandle> GraphContext::get_input_buffer(
    const SlotLabel& label) const {
    auto value = get_input(label);
...
EPIX_API std::optional<nvrhi::TextureHandle> GraphContext::get_input_texture(
    const SlotLabel& label) const {
    auto value = get_input(label);
...
EPIX_API std::optional<nvrhi::SamplerHandle> GraphContext::get_input_sampler(
    const SlotLabel& label) const {
    auto value = get_input(label);
...
EPIX_API bool GraphContext::set_output(const SlotLabel& label,
...
EPIX_API epix::app::Entity GraphContext::view_entity() const {
    return m_view_entity.value();
}
EPIX_API std::optional<epix::app::Entity> GraphContext::get_view_entity()
    const {
    return m_view_entity;
}
EPIX_API void GraphContext::set_view_entity(epix::app::Entity entity) {
    m_view_entity = entity;
}

EPIX_API bool GraphContext::run_sub_graph(
...
EPIX_API std::vector<RunSubGraph> GraphContext::finish() {
