...
EPIX_API RenderContext::RenderContext(nvrhi::DeviceHandle device)
    : m_device(device) {}
...
EPIX_API nvrhi::CommandListHandle RenderContext::begin_render_pass(
    const nvrhi::GraphicsState& state) {
    auto cmd_list = commands();
    cmd_list->setGraphicsState(state);
    return cmd_list;
}
EPIX_API void RenderContext::add_command_list(
