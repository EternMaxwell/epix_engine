...
EPIX_API void WindowRenderPlugin::build(epix::App& app) {
    ...
    render_app.add_systems(epix::render::ExtractSchedule, epix::into(extract_windows).set_names({"extract windows"}));
    render_app.add_systems(epix::render::Render,
                           epix::into(create_surfaces).set_names({"create_surfaces"}).before(prepare_windows));
...
EPIX_API void epix::render::window::create_surfaces(Res<ExtractedWindows> windows,
                                                    ResMut<WindowSurfaces> window_surfaces,
