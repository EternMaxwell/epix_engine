...
EPIX_API std::optional<SpritePipeline> SpritePipeline::from_world(World& world) {
...
EPIX_API void SpriteShadersPlugin::build(App& app) {
    app.add_systems(
...
EPIX_API std::optional<VertexBuffers> VertexBuffers::from_world(World& world) {
...
EPIX_API void SpriteInstanceBuffer::upload(nvrhi::DeviceHandle device, nvrhi::CommandListHandle cmd_list) {
...
EPIX_API void SpriteInstanceBuffer::upload(nvrhi::DeviceHandle device) {
    auto cmd_list = device->createCommandList(nvrhi::CommandListParameters().setEnableImmediateExecution(false));
    cmd_list->open();
...
EPIX_API void DefaultSamplerPlugin::build(App& app) {
...
EPIX_API void sprite::extract_sprites(
    Commands cmd,
...
EPIX_API void sprite::queue_sprites(
    Query<Item<Mut<render::render_phase::RenderPhase<render::core_2d::Transparent2D>>>,
...
EPIX_API void sprite::create_uniform_for_view(
    Commands cmd,
...
EPIX_API void sprite::prepare_sprites(
    Query<Item<Mut<render::render_phase::RenderPhase<render::core_2d::Transparent2D>>>,
...
EPIX_API void SpritePlugin::build(App& app) {
