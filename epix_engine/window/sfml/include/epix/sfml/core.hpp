...
EPIX_EXPORT struct SFMLwindows : public std::unordered_map<Entity, std::shared_ptr<sf::WindowBase>> {};
...
EPIX_EXPORT struct PendingWindowPositions : public std::unordered_map<Entity, PendingWindowPosition> {};
 ...
 * `set_render_app()`.
 */
EPIX_EXPORT struct SFMLRunner : public AppRunner {
   public:
    SFMLRunner(App& app);
