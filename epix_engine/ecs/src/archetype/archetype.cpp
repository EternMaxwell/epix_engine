...
Archetype Archetype::create(internal::ComponentIndex& component_index,
    ...
    for (auto&& [idx, type_id] : std::views::enumerate(table_components)) {
        ...
        component_index[type_id][id] = internal::ArchetypeRecord{static_cast<size_t>(idx)};
    }
    for (auto&& type_id : sparse_components) {
        arch._components.emplace(type_id, StorageType::SparseSet);
        component_index[type_id][id] = internal::ArchetypeRecord{std::nullopt};
    }
    return arch;
}

internal::ArchetypeSwapRemoveResult Archetype::swap_remove(ArchetypeRow arch_idx) {
...
std::pair<ArchetypeId, bool> Archetypes::get_id_or_insert(TableId table_id,
                                                          std::vector<TypeId> table_components,
                                                          std::vector<TypeId> sparse_components) {
    internal::ArchetypeComponents archetype_components{
