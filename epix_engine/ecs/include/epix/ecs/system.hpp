...
EPIX_EXPORT struct SystemException {
...
EPIX_EXPORT template <typename In, typename Out>
struct System {
    ...
    virtual std::expected<Out, RunSystemError> run_internal(typename internal::SystemInput<In>::Input input,
                                                            World& world) = 0;
    ...
    std::expected<Out, RunSystemError> run(typename internal::SystemInput<In>::Input input, World& world) {
    ...
    std::expected<Out, RunSystemError> run_no_apply(typename internal::SystemInput<In>::Input input, World& world) {
...
EPIX_EXPORT template <typename In = std::tuple<>, typename Out = void>
...
struct function_system_traits {
    using Traits                       = traits::function_traits<F>;
    ...
    using Input                     = std::remove_reference_t<decltype(*([] {
        if constexpr (arity == 0) {
            ...
            if constexpr (internal::system_input<FirstArg>) {
...
concept valid_function_system = requires {
    ...
    requires internal::system_input<typename function_system_traits<F>::Input>;
