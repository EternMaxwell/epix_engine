...
// hash for Label
template <>
struct std::hash<::epix::ecs::Label> {
    std::size_t operator()(const ::epix::ecs::Label& label) const noexcept {
...
template <std::derived_from<::epix::ecs::Label> T>
struct std::hash<T> {
    std::size_t operator()(const T& label) const noexcept {
        return std::hash<::epix::ecs::Label>()(static_cast<const ::epix::ecs::Label&>(label));
    }
};