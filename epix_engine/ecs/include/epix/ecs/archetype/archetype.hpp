...
EPIX_EXPORT struct Archetype {
   public:
    ...
    static Archetype create(internal::ComponentIndex& component_index,
                            ArchetypeId id,
                            TableId table_id,
                            std::vector<TypeId> table_components,
                            std::vector<TypeId> sparse_components);
    ...
    std::span<const internal::ArchetypeEntity> entities() const noexcept { return _entities; }
    ...
    const internal::ArchetypeEdges& edges() const noexcept { return _edges; }
    /** @brief Get a mutable reference to the archetype's edge cache. */
    internal::ArchetypeEdges& edges_mut() noexcept { return _edges; }
    ...
    internal::ArchetypeSwapRemoveResult swap_remove(ArchetypeRow arch_idx);
...
EPIX_EXPORT struct Archetypes {
    ...
    std::unordered_map<internal::ArchetypeComponents,
                       ArchetypeId,
                       internal::ArchetypeComponentsHash,
                       internal::ArchetypeComponentsEqual>
        by_components;
    internal::ComponentIndex by_component;

    Archetypes() {
        // default add an empty archetype at index 0
        archetypes.emplace_back(Archetype::empty({0}));
        by_components.insert({internal::ArchetypeComponents{{}, {}}, {0}});
    }
