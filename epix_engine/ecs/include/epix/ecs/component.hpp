        ...
        register_dynamic(type_id, inheritance_depth,
                         std::make_shared<std::function<void(Table&, SparseSets&, Tick, TableRow, Entity)>>(
                             [ctor = std::forward<F>(constructor), type_id](Table& table, SparseSets& sparse_sets,
                                 ...
                                 auto storage_type = storage_type_of<C>();
...
EPIX_EXPORT struct Components : public internal::SparseSet<TypeId, internal::ComponentInfo> {
   ...
   public:
    Components(std::shared_ptr<TypeRegistry> type_registry) noexcept
        : SparseSet<TypeId, internal::ComponentInfo>(), type_registry(std::move(type_registry)) {}
    ...
    TypeId register_info() {
        ...
        if (!contains(id)) {
            auto info = internal::ComponentInfo(id, type_registry->type_index(id), type_registry->storage_type(id));
            info.template update_hooks<T>();
    ...
    void update_hooks(TypeId type_id, ComponentHooks hooks) noexcept {
        get_mut(type_id).transform([&](internal::ComponentInfo& info) -> bool {
            if (hooks.on_add) {
        ...
        requires std::invocable<F> && std::is_object_v<std::invoke_result_t<F>>
    {
        ...
        internal::RequiredComponents required_components_tmp;
    ...
    void register_required_dyn(TypeId requiree, TypeId required, internal::RequiredComponentConstructor constructor) {
        auto& required_components = get_mut(requiree).value().get()._required_components;
        auto existing_required    = required_components.components.find(required);
        if (existing_required != required_components.components.end() &&
            existing_required->second.inheritance_depth == 0) {
   ...
   private:
    std::vector<std::pair<TypeId, internal::RequiredComponent>> register_inherited_required_components(
        TypeId requiree, TypeId required, internal::RequiredComponents& required_components) {
        auto& required_info = get_mut(required).value().get();
        std::vector<std::pair<TypeId, internal::RequiredComponent>> inherited_requirements =
            std::ranges::to<std::vector<std::pair<TypeId, internal::RequiredComponent>>>(
                std::views::transform(required_info.required_components().components, [&](auto&& rc) {
                    auto&& [type_id, req_comp] = rc;
                    return std::pair(
                        type_id, internal::RequiredComponent{
