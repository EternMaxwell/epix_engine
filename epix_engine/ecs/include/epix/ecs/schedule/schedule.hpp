...
EPIX_EXPORT struct Node {
    ...
    internal::Edges validated_edges;
};
EPIX_EXPORT struct CachedNode {
...
EPIX_EXPORT struct SetConfig {
   ...
   private:
    ...
    friend SetConfig single_set(F&& func)
        requires(!require_system || internal::valid_system<F> || std::same_as<std::decay_t<F>, SetConfig>);
    template <bool require_system, typename... Ts>
    friend SetConfig make_sets(Ts&&... ts)
        requires(sizeof...(Ts) >= 1) &&
                (... && (!require_system || internal::valid_system<Ts> || std::same_as<std::decay_t<Ts>, SetConfig>));
};
EPIX_EXPORT template <bool require_system, typename F>
...
EPIX_EXPORT template <bool require_system, typename... Ts>
...
EPIX_EXPORT enum class DeferredApply {
...
EPIX_EXPORT struct ExecutorConfig {
...
EPIX_EXPORT struct ScheduleConfig {
...
EPIX_EXPORT struct ScheduleSystems {
...
EPIX_EXPORT struct ScheduleExecutor {
    ScheduleLabel label;
...
SetConfig into(Ts&&... ts)
    requires(sizeof...(Ts) >= 1) && (... && (internal::valid_system<Ts> || std::same_as<std::decay_t<Ts>, SetConfig>))
{
