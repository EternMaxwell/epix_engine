...
EPIX_API void ScheduleProfiler::reset() {
    ...
    m_flush_time_last   = 0.0;
    m_flush_time_avg    = 0.0;
    m_build_time_last   = 0.0;
    m_build_time_avg    = 0.0;
    m_prepare_time_last = 0.0;
    m_prepare_time_avg  = 0.0;
    m_run_time_last     = 0.0;
    m_run_time_avg      = 0.0;
...
EPIX_API double ScheduleProfiler::flush_time_last() const {
    return m_flush_time_last;
}
EPIX_API double ScheduleProfiler::flush_time_avg() const {
    return m_count ? m_flush_time_avg / (1 - m_empty_zone) : m_flush_time_avg;
}
...
EPIX_API double ScheduleProfiler::prepare_time_last() const {
    return m_prepare_time_last;
}
EPIX_API double ScheduleProfiler::prepare_time_avg() const {
    return m_count ? m_prepare_time_avg / (1 - m_empty_zone)
                   : m_prepare_time_avg;
}
...
EPIX_API void ScheduleProfiler::push_time(
    double flush_time, double build_time, double prepare_time, double run_time
) {
    m_count++;

    m_flush_time_last = flush_time;
    m_flush_time_avg =
        m_flush_time_avg * (1.0 - m_factor) + flush_time * m_factor;
    m_build_time_last = build_time;
    m_build_time_avg =
        m_build_time_avg * (1.0 - m_factor) + build_time * m_factor;
    m_prepare_time_last = prepare_time;
    m_prepare_time_avg =
        m_prepare_time_avg * (1.0 - m_factor) + prepare_time * m_factor;
