 ...
 */
EPIX_EXPORT struct TaskPoolThreadAssignmentPolicy {
 ...
 */
EPIX_EXPORT struct TaskPoolOptions {
    ...
    void create_default_pools() const {
        ...
        task::IoTaskPool::get_or_init([io_threads] {
            ...
            return task::TaskPoolBuilder{}
                .num_threads(io_threads)
                .thread_name("IO Task Pool")
                .backend(task::TaskPoolBackend::IoContext)
                .build();
        });
        ...
        task::AsyncComputeTaskPool::get_or_init([async_threads] {
            return task::TaskPoolBuilder{}.num_threads(async_threads).thread_name("Async Compute Task Pool").build();
        });
        ...
        task::ComputeTaskPool::get_or_init([compute_threads] {
            return task::TaskPoolBuilder{}.num_threads(compute_threads).thread_name("Compute Task Pool").build();
        });
    }
};
 ...
 */
EPIX_EXPORT struct TaskPoolPlugin {
