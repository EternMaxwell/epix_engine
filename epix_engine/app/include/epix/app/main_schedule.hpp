...
EPIX_EXPORT struct ScheduleInfo : public ecs::ScheduleLabel {
    using ScheduleLabel::ScheduleLabel;

    /** @brief Transforms applied to system sets added under this schedule label. */
    std::vector<std::function<void(ecs::SetConfig&)>> transforms;
};
...
EPIX_EXPORT inline struct OnEnterT {
    ...
    ScheduleInfo operator()(T state) {
        ...
        info.transforms.emplace_back([state](ecs::SetConfig& info) {
            info.run_if(
                [state](ecs::Res<State<T>> cur) { return (*cur == state) && (cur.is_added() || cur.is_modified()); });
...
EPIX_EXPORT inline struct OnChangeT {
    ...
    ScheduleInfo operator()(T state = T{}) {
        ScheduleInfo info(StateTransition);
        info.transforms.emplace_back([](ecs::SetConfig& info) {
            info.run_if([](ecs::Res<State<T>> cur) { return cur.is_modified(); });
            info.in_set(StateTransitionSet::Callback);
        });
        return info;
    }
