...
void camera_control_bm4d(Res<input::ButtonInput<input::KeyCode>> keys,
                         ...
                         ecs::ResMut<BM4DCameraState> cam) {
...
void bm4d_imgui_ui(imgui::Ctx /*ctx*/, ecs::ResMut<BM4DConfig> cfg, ecs::ResMut<BM4DCameraState> cam) {
...
void setup_bm4d_scene(Commands cmd) {
    ...
    cmd.spawn(::epix::camera::Camera{}, CameraRenderGraph(BM4DGraph), std::move(projection),
              tf::Transform::from_xyz(16.0f, 12.0f, -30.0f));
}

void extract_bm4d_scene(Commands cmd, Extract<Query<Item<const BM4DScene&>>> scenes, Local<bool> extracted) {
    // The demo scene is immutable after startup, so extract it once.
    if (*extracted) return;
...
struct BM4DPathTracerPlugin {
    void attach(app::App& app) {
        ...
        app.add_systems(app::Startup, into(setup_bm4d_scene).set_name("setup bm4d scene"));
        app.add_systems(app::Update, into(camera_control_bm4d).set_name("camera control bm4d"));
        app.add_systems(app::Update, into(bm4d_imgui_ui).set_name("bm4d imgui ui"));
    }
