...
void traceMain(uint3 id : SV_DispatchThreadID) {
    ...
    // near plane, while depth 0 is the point at infinity.
...
void extract_voxel_scene(Commands cmd, Extract<Query<Item<const VoxelScene&>>> scenes, Local<bool> extracted) {
    // This example's scene is static.  Rebuilding its CPU BrickMap every
    // extraction frame was the cause of the very low frame rate.
    if (*extracted) return;
...
struct BrickmapPathTracerPlugin {
    void attach(app::App& app) {
        ...
        app.add_systems(app::Startup, into(setup_voxel_scene).set_name("setup brickmap scene"));
        app.add_systems(app::Update, into(camera_control).set_name("camera control"));
        app.add_systems(app::Update, into(voxel_imgui_ui).set_name("brickmap imgui ui"));
    }
