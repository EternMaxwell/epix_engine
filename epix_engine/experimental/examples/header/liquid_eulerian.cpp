...
struct GpuPressureProjector {
    ...
    void init(const wgpu::Device& device) {
        ...
        const auto storage = [](std::uint32_t binding, wgpu::BufferBindingType type) {
            ...
            // A uniform binding has a statically known 32-byte SimParam
            // layout. wgpu-native rejects a pipeline layout whose uniform
            // binding leaves its minimum size as the generated descriptor's
            // zero default.
