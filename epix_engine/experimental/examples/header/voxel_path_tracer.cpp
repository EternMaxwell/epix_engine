...
void traceMain(uint3 id : SV_DispatchThreadID) {
    ...
    // Bevy's perspective projection is reverse-Z: depth 1 is the finite
    // near plane, while depth 0 is the point at infinity.  Reconstruct the
    // ray from the near plane instead of dividing the far-plane direction by
    // zero.
    float4 vr = mul(camera.inv_proj, float4(ndc, 1.0f, 1.0f));
    vr.xyz /= vr.w;
...
float3 reproject(float2 uv, float depth, float4x4 inv_proj, float4x4 inv_view) {
    ...
    float4 vp  = mul(inv_proj, float4(ndc, 1.0f, 1.0f));
    vp.xyz /= vp.w;
...
void camera_control(Res<input::ButtonInput<input::KeyCode>> keys,
    ...
    for (auto&& [transform_mut] : cameras.iter()) {
        ...
        glm::vec3 move{0.0f};
        // Bevy cameras look along local -Z.
        const glm::vec3 forward = -tr.local_z();
        if (keys->pressed(input::KeyCode::KeyW)) move += forward;
        if (keys->pressed(input::KeyCode::KeyS)) move -= forward;
...
void setup_voxel_scene(Commands cmd) {
    ...
    // Bevy cameras look along local -Z.  Rotate the camera to look toward the
    // positive-Z scene from outside its near edge.
    auto camera_transform = tf::Transform::from_xyz(40.0f, 15.0f, -30.0f);
    camera_transform.rotate_y(glm::radians(180.0f));
    cmd.spawn(::epix::camera::Camera{}, CameraRenderGraph(VoxelGraph), std::move(projection),
              std::move(camera_transform));
}

void extract_voxel_scene(Commands cmd, Extract<Query<Item<const VoxelScene&>>> scenes, Local<bool> extracted) {
    // This example's scene is static.  Rebuilding its CPU SVO on every
    // extraction frame needlessly consumed several cores.
    if (*extracted) return;
...
struct VoxelPathTracerPlugin {
    ...
    void ready(app::App& app) {
        ...
        rapp.add_systems(ExtractSchedule, into(extract_voxel_scene).set_name("extract voxel scene"))
            ...
            .add_systems(
                Render,
                into(prepare_voxel_render).in_set(RenderSystems::PrepareResources).set_name("prepare voxel render"));
    }
};
