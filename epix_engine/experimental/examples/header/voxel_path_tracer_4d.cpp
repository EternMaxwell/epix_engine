...
void setup_v4d_scene(Commands cmd) {
    ...
    cmd.spawn(::epix::camera::Camera{}, CameraRenderGraph(V4DGraph), std::move(projection),
              tf::Transform::from_xyz(16.0f, 12.0f, -6.0f));
}

void extract_v4d_scene(Commands cmd, Extract<Query<Item<const Voxel4DScene&>>> scenes, Local<bool> extracted) {
