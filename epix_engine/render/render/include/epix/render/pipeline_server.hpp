 ...
 * layout IDs. */
using LayoutCacheKey   = std::vector<wgpu::BindGroupLayoutId>;
using CachedPipelineId = shader::CachedPipelineId;
 ...
 * descriptor. */
using PipelineDescriptor = std::variant<RenderPipelineDescriptor, ComputePipelineDescriptor>;
/** @brief Variant holding a created render or compute pipeline. */
using Pipeline = std::variant<RenderPipeline, ComputePipeline>;
 ...
 * operations. */
using PipelineServerError = std::variant<PipelineError, shader::ShaderCacheError>;
/** @brief Returned when a queried pipeline is still queued or being
 * compiled. */
struct GetPipelineNotReady {};
/** @brief Returned when a pipeline ID is out of range. */
struct GetPipelineInvalidId {};
/** @brief Error variant returned by pipeline retrieval methods. */
using GetPipelineError = std::variant<GetPipelineNotReady, GetPipelineInvalidId, PipelineServerError>;
struct PipelineStateQueued {};
using PipelineStateCreating = std::future<std::expected<Pipeline, PipelineServerError>>;
struct PipelineStateRecoverableShaderError {
