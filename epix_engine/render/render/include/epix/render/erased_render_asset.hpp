 ...
 * `ErasedAsset`, `Param`, and the static-like `prepare_asset` member to
 * make source assets extractable and processable as type-erased GPU render
 * assets (Bevy `ErasedRenderAsset`).
...
concept ErasedRenderAssetImpl = requires(ErasedRenderAsset<A> asset) {
    ...
    {
        asset.prepare_asset(std::declval<typename ErasedRenderAsset<A>::SourceAsset&&>(),
                            ...
                            std::declval<typename ErasedRenderAsset<A>::Param&>())
                                    ...
                                    PrepareAssetError<typename ErasedRenderAsset<A>::SourceAsset>>>;
};
...
std::optional<std::size_t> erased_asset_byte_len(const ErasedRenderAsset<A>& asset,
                                                 const typename ErasedRenderAsset<A>::SourceAsset& source) noexcept {
    if constexpr (HasErasedByteLen<A>) {
        return asset.byte_len(source);
    } else {
        (void)asset;
        (void)source;
        return std::nullopt;
...
struct ExtractedAssets {
    ...
    std::vector<std::pair<assets::AssetId<SourceAsset>, SourceAsset>> extracted;
...
struct PrepareNextFrameAssets {
    ...
    std::vector<std::pair<assets::AssetId<SourceAsset>, SourceAsset>> assets;
};
...
void extract_erased_render_asset(
    ...
    for (const auto& event : events.read()) {
        if (event.is_added()) {
            needs_extracting.insert(event.id);
            ...
            needs_extracting.erase(event.id);
...
void prepare_erased_assets(typename ErasedRenderAsset<A>::Param param,
    ...
    auto write_bytes = [&](const SourceAsset& asset) -> std::optional<std::size_t> {
