...
struct epix::render::RenderAsset<IncrementalExtractSource> {
    ...
    std::expected<ExtractedAsset, ExtractError> extract(const IncrementalExtractSource& source,
        ...
        const bool full_snapshot = reason == RenderAssetExtractionReason::Added || previous_asset == nullptr;
...
TEST(RenderCameraPlugin, CameraMarkersInheritRegisteredMsaaRequirement) {
    epix::ecs::World world(2);
    world.register_required_components_with<::epix::camera::Camera>([] { return view::Msaa::Sample4; });
    ...
    for (Entity entity : {raw_camera, camera2d, camera3d}) {
        EXPECT_EQ(world.entity(entity).get<view::Msaa>()->get(), view::Msaa::Sample4);
    }
}

TEST(SortedCamera, TargetAlternativesSortDistinctly) {
    const auto manual = ::epix::camera::RenderTarget::from_manual_texture_view(
        ::epix::camera::ManualTextureViewHandle{42});
...
TEST(RenderAsset, NewAssetExtractsCompletePayload) {
    IncrementalExtractSource source{
        .resident_data = std::vector<std::uint32_t>(4096, 7),
        .dirty_region  = {17, 19, 23},
    };
    ...
    const auto payload = asset.extract(source,
                                       epix::assets::AssetId<IncrementalExtractSource>::invalid(),
                                       RenderAssetExtractionReason::Added,
                                       nullptr);

    ASSERT_TRUE(payload.has_value());
    EXPECT_EQ(payload->values, source.resident_data);
    EXPECT_TRUE(payload->full_snapshot);
    EXPECT_EQ(payload->reason, RenderAssetExtractionReason::Added);
