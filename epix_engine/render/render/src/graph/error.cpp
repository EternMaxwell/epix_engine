...
std::string RenderGraphRunnerError::to_string() const {
    return std::visit(epix::utils::visitor{
                          [](const RunnerNodeRunError& e) {
                              return std::format("node '{}' failed with NodeRunError {}",
                                                 e.node.type_index().short_name(), static_cast<std::uint32_t>(e.error));
                          },
                          [](const RunnerMissingInput& e) {
                              return std::format("graph '{}' input slot {} ('{}') has no value",
                                                 e.sub_graph ? e.sub_graph->type_index().short_name() : "main",
                                                 e.slot_index, e.slot_name);
                          },
                          ...
                          [](const RunnerSubGraphNotFound& e) {
                              return std::format("queued subgraph '{}' was not found",
                                                 e.sub_graph.type_index().short_name());
                          },
                      },
                      *this);
}
