...
std::expected<void, GraphError> RenderGraph::remove_node(const NodeLabel& id) {
    if (auto state = get_node_state(id)) {
        std::ranges::for_each(state->get().edges.input_edges(), [&](const Edge& edge) {
            get_node_state(edge.output_node).transform([&](NodeState& output_node) {
                output_node.edges.remove_output_edge(edge);
                return std::ref(output_node);
            });
        });
        std::ranges::for_each(state->get().edges.output_edges(), [&](const Edge& edge) {
            get_node_state(edge.input_node).transform([&](NodeState& input_node) {
                input_node.edges.remove_input_edge(edge);
                return std::ref(input_node);
            });
        });
        nodes.erase(id);
        return {};
...
std::expected<void, EdgeError> RenderGraph::validate_edge(const Edge& edge, bool should_exist) {
    ...
    auto from_slot = output_node->get().outputs.get_slot(edge.output_index);
    auto to_slot   = input_node->get().inputs.get_slot(edge.input_index);
    ...
    if (from_slot->get().type != to_slot->get().type) {
        return std::unexpected(SlotTypeMismatch{
            .output_node  = edge.output_node,
            .output_index = edge.output_index,
            .input_node   = edge.input_node,
            .input_index  = edge.input_index,
            .output_type  = from_slot->get().type,
            .input_type   = to_slot->get().type,
        });
    }
...
std::expected<void, GraphError> RenderGraph::try_add_node_edge(const NodeLabel& output_node,
    ...
    output_node_state->get().edges.add_output_edge(edge);
    input_node_state->get().edges.add_input_edge(edge);
...
std::expected<void, GraphError> RenderGraph::try_add_slot_edge(const NodeLabel& output_node,
    ...
    auto output_index = output_node_state->get().outputs.get_slot_index(output_slot);
    auto input_index  = input_node_state->get().inputs.get_slot_index(input_slot);
...
std::optional<std::reference_wrapper<const NodeState>> RenderGraph::get_node_state(const NodeLabel& id) const {
    auto iter = nodes.find(id);
    if (iter != nodes.end()) {
        return std::cref(iter->second);
    }
...
std::optional<std::reference_wrapper<const RenderGraph>> RenderGraph::get_sub_graph(const GraphLabel& id) const {
    auto iter = sub_graphs.find(id);
    if (iter != sub_graphs.end()) {
        return std::cref(iter->second);
    }
