    ...
    struct Command {
        ...
        bool render(const P& item,
            ...
            binding_set                     = device.get()->createBindingSet(
                nvrhi::BindingSetDesc().addItem(nvrhi::BindingSetItem::ConstantBuffer(
...
struct DrawMesh {
    ...
    bool render(const P& item,
                Item<> entity_item,
                std::optional<Item<>> view_item,
                ParamSet<Res<render::assets::RenderAssets<Mesh>>> params,
                render::render_phase::DrawContext& ctx) {
        auto&& [render_assets]  = params.get();
        auto entity             = item.entity();
        const GPUMesh* gpu_mesh = render_assets->try_get(entity);
