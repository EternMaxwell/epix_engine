...
// C++20 module that provides 3D transform types and utilities

module;

// Global module fragment - include headers that can't be modularized
#include <glm/glm.hpp>
...
// Import dependencies - note: these would need to be modules too
// For now, we'll use the global module fragment for epix::core
import epix.core;
...
struct TransformT {
    ...
    TransformT mul_transform(this const TransformT& t, const TransformT& other) { 
        return t * other; 
    }
...
#else
    // C++20 fallback without deducing this
    TransformT& look_at(const glm::vec3& target, const glm::vec3& up) {
        glm::vec3 direction = glm::normalize(target - translation);
        rotation = glm::quatLookAt(direction, up);
        return *this;
    }
    
    TransformT& look_to(const glm::vec3& direction, const glm::vec3& up) {
        rotation = glm::quatLookAt(direction, up);
        return *this;
    }

    glm::mat4 to_matrix() const {
        glm::mat3 rotate = glm::mat3_cast(rotation);
        glm::mat4 matrix(glm::vec4(rotate[0] * scaler.x, 0.0f), 
                         glm::vec4(rotate[1] * scaler.y, 0.0f),
                         glm::vec4(rotate[2] * scaler.z, 0.0f), 
                         glm::vec4(translation, 1.0f));
        return matrix;
    }

    glm::vec3 local_x() const { 
        return glm::normalize(rotation * glm::vec3(1.0f, 0.0f, 0.0f)); 
    }
    
    glm::vec3 local_y() const { 
        return glm::normalize(rotation * glm::vec3(0.0f, 1.0f, 0.0f)); 
    }
    
    glm::vec3 local_z() const { 
        return glm::normalize(rotation * glm::vec3(0.0f, 0.0f, 1.0f)); 
    }

    TransformT& rotate(const glm::quat& r) {
        rotation = r * rotation;
        return *this;
    }
    
    TransformT& rotate(const glm::vec3& axis, float angle) { 
        return rotate(glm::angleAxis(angle, axis)); 
    }
    
    // ... (rest of methods without deducing this)
    
    TransformT operator*(const TransformT& other) const {
        TransformT result = *this;
        result.translation = other * result.translation;
        result.rotation    = other.rotation * result.rotation;
        result.scaler *= other.scaler;
        return result;
    }
    
    glm::vec3 operator*(const glm::vec3& v) const {
        return translation + rotation * (v * scaler);
    }
#endif
};
