...
struct TransformT {
    ...
    static TransformT identity() noexcept { return TransformT(); }
    /** @brief Decompose a 4x4 matrix into translation, rotation, and scale. */
    static TransformT from_matrix(const glm::mat4& matrix) noexcept {
        TransformT t;
    ...
    static TransformT from_translation(const glm::vec3& translation) noexcept {
        TransformT t;
        t.translation = translation;
        return t;
    }
    /** @brief Create a transform from x, y, z translation. */
    static TransformT from_xyz(float x, float y, float z) noexcept { return from_translation(glm::vec3(x, y, z)); }
    /** @brief Create a transform with only rotation set. */
    static TransformT from_rotation(const glm::quat& rotation) noexcept {
        TransformT t;
        t.rotation = rotation;
        return t;
    }
    /** @brief Create a transform with only scale set. */
    static TransformT from_scale(const glm::vec3& scale) noexcept {
        TransformT t;
        t.scaler = scale;
        return t;
    }
    ...
    auto&& translate(this auto&& t, float x, float y, float z) noexcept { return t.translate(glm::vec3(x, y, z)); }
    /** @brief Multiply current scale by a scale vector. */
    auto&& scale(this auto&& t, const glm::vec3& scale) noexcept {
        t.scaler *= scale;
        return std::forward<decltype(t)>(t);
    }
    /** @brief Multiply current scale by x, y, z components. */
    auto&& scale(this auto&& t, float x, float y, float z) noexcept { return t.scale(glm::vec3(x, y, z)); }

    /** @brief Translate the position by rotating around a point. */
    auto&& translate_around(this auto&& t, const glm::vec3& point, const glm::quat& rotation) noexcept {
        t.translation = point + rotation * (t.translation - point);
        return std::forward<decltype(t)>(t);
    }
    /** @brief Rotate the transform around a world-space point. */
    auto&& rotate_around(this auto&& t, const glm::vec3& point, const glm::quat& rotation) noexcept {
    ...
    TransformT operator*(this TransformT t, const TransformT& other) noexcept {
    ...
    glm::vec3 operator*(this const TransformT& t, const glm::vec3& v) noexcept {
        return t.translation + t.rotation * (v * t.scaler);
    }

    /** @brief Compose with another transform. */
    TransformT mul_transform(this const TransformT& t, const TransformT& other) noexcept { return t * other; }
    /** @brief Transform a point. */
    glm::vec3 mul_vec3(this const TransformT& t, const glm::vec3& v) noexcept { return t * v; }
};
