...
TEST(utils, bitvector) {
    // basic construction and queries
    bit_vector b(130, false);
    EXPECT_TRUE(b.size() == 130);
    EXPECT_TRUE(b.count_ones() == 0);
    ...
    EXPECT_TRUE(b.count_ones() == b.size());
    EXPECT_TRUE(b.contains_all_in_range(0, b.size()));
    b.reset_all();
    EXPECT_TRUE(b.count_ones() == 0);
    // is_clear should be true after reset
    EXPECT_TRUE(b.is_clear());
    ...
    EXPECT_TRUE(c.is_subset(a));
    EXPECT_TRUE(a.is_superset(c));
    EXPECT_TRUE(!a.is_subset(c));
    EXPECT_TRUE(!c.is_superset(a));
    bit_vector d(16, false), e(16, false);
    d.set(1);
    e.set(2);
    EXPECT_TRUE(d.is_disjoint(e));
    ...
    exp_or.set(4);
    exp_or.set(5);
    EXPECT_TRUE(orv == exp_or);

    auto xorv = x.bit_xor(y);
    bit_vector exp_xor(16, false);
    exp_xor.set(1);
    exp_xor.set(4);
    EXPECT_TRUE(xorv == exp_xor);

    // assign variants
    bit_vector xa = x;
    xa.bit_or_assign(y);
    EXPECT_TRUE(xa == exp_or);
    xa = x;
    xa.bit_and_assign(y);
    EXPECT_TRUE(xa == exp_and);
    xa = x;
    xa.bit_xor_assign(y);
    EXPECT_TRUE(xa == exp_xor);

    // contains_any_in_range and contains_all_in_range
    bit_vector r(32, false);
    r.set_range(4, 8);                           // set 4..7
    EXPECT_TRUE(r.contains_any_in_range(0, 5));  // intersects at 4
    EXPECT_TRUE(r.contains_all_in_range(4, 8));
    ...
    smallgrow.set(10);
    EXPECT_TRUE(smallgrow.size() == 11);
    EXPECT_TRUE(smallgrow.test(10));

    // std::hash usage in unordered_set
    std::unordered_set<bit_vector> set;
    set.insert(p);
    EXPECT_TRUE(set.find(p) != set.end());
}