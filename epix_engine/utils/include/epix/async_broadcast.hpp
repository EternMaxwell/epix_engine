...
EPIX_EXPORT enum class RecvError {
    Closed,
};
...
struct Sender {
   private:
    std::shared_ptr<internal::BroadcastState<T>> m_st;
   ...
   public:
    ...
    explicit Sender(std::shared_ptr<internal::BroadcastState<T>> st) : m_st(std::move(st)) { inc(); }
...
struct Receiver {
   ...
   public:
    ...
    explicit Receiver(std::shared_ptr<internal::BroadcastState<T>> st, std::size_t cursor)
...
struct InactiveReceiver {
   ...
   public:
    ...
    InactiveReceiver(std::shared_ptr<internal::BroadcastState<T>> st, std::size_t cursor) noexcept
        : m_st(std::move(st)), m_cursor(cursor) {}
...
std::pair<Sender<T>, Receiver<T>> broadcast(std::size_t cap) {
    auto st      = std::make_shared<internal::BroadcastState<T>>();
    st->capacity = cap;
    Sender<T> s(st);
    Receiver<T> r(st, 0);
