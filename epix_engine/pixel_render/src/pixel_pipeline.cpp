...
EPIX_API PixelPipeline::PixelPipeline(Device device) : device(device) {}

EPIX_API void PixelPipeline::create() {
...
EPIX_API void PixelPipeline::destroy() {
...
EPIX_API PixelPipeline::Context PixelPipeline::create_context(
    ...
    auto vertex_buffer = device.createBuffer(
        vk::BufferCreateInfo()
            .setSize(sizeof(PixelVertex) * max_vertex_count)
            .setUsage(
...
EPIX_API void PixelPipeline::destroy_context(
...
EPIX_API void PixelPipeline::create_descriptor_set_layout() {
...
EPIX_API void PixelPipeline::create_render_pass() {
...
EPIX_API void PixelPipeline::create_pipeline() {
    using namespace epix::render::vulkan2::util;
    auto& vertex_spv   = pixel_vk_vertex_spv;
    auto& fragment_spv = pixel_vk_fragment_spv;
    auto& geometry_spv = pixel_vk_geometry_spv;
    ...
    auto geom_source = std::vector<uint32_t>(
        geometry_spv, geometry_spv + sizeof(geometry_spv) / sizeof(uint32_t)
    );
    auto geom_module = device.createShaderModule(
        vk::ShaderModuleCreateInfo().setCode(geom_source)
    );
    spirv_cross::CompilerGLSL vert(vert_source);
    spirv_cross::CompilerGLSL frag(frag_source);
    spirv_cross::CompilerGLSL geom(geom_source);
    ...
    auto bindings          = std::vector<vk::VertexInputBindingDescription>{
        vk::VertexInputBindingDescription().setStride(sizeof(PixelVertex))
    };
    ...
    auto stages = default_shader_stages(
        ...
        vk::ShaderStageFlagBits::eGeometry, geom_module,
    ...
    device.destroyShaderModule(geom_module);
}
