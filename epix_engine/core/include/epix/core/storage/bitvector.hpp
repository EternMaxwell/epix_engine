...
// A FixedBitSet-like dynamic bitset tuned for ECS masks.
// Provides named operations and auto-growing modifiers.
...
namespace std {
template <>
struct hash<epix::core::storage::bit_vector> {
    size_t operator()(epix::core::storage::bit_vector const& bv) const noexcept {
        using bv_t        = epix::core::storage::bit_vector;
        size_t h          = bv.size();
        const auto& words = bv.words();
        for (auto w : words) {
            h = (h * 0x9e3779b97f4a7c15ULL) ^ static_cast<size_t>(w + 0x9e3779b97f4a7c15ULL + (h << 6) + (h >> 2));
        }
        return h;
    }
};
