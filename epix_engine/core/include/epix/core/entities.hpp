...
EPIX_EXPORT EPIX_MAKE_INT_WRAPPER(ArchetypeId, std::uint32_t);
/** @brief Strongly-typed table identifier. */
EPIX_EXPORT EPIX_MAKE_INT_WRAPPER(TableId, std::uint32_t);
/** @brief Strongly-typed row index within an archetype's entity list. */
EPIX_EXPORT EPIX_MAKE_INT_WRAPPER(ArchetypeRow, std::uint32_t);
/** @brief Strongly-typed row index within a table. */
EPIX_EXPORT EPIX_MAKE_INT_WRAPPER(TableRow, std::uint32_t);
...
struct hash<::epix::core::ArchetypeId> {
    std::size_t operator()(const ::epix::core::ArchetypeId& id) const noexcept {
        return std::hash<::epix::utils::int_base<std::uint32_t>>()(id);
    }
};
template <>
...
struct hash<::epix::core::BundleId> {
    std::size_t operator()(const ::epix::core::BundleId& id) const noexcept {
        return std::hash<::epix::utils::int_base<std::uint64_t>>()(id);
    }
};
template <>
