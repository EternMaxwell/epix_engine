       ...
       public:
            ...
            requires std::ranges::range<Rng> && std::same_as<std::ranges::range_value_t<Rng>, ScheduleLabel>
        {
            ...
            labels.insert_range(labels.end(),
                                std::views::filter(std::forward<Rng>(new_labels), [&](const ScheduleLabel& label) {
                                    return !existing.contains(label);
                                }));
        }
    ...
    struct App {
       public:
        ...
        std::optional<std::reference_wrapper<const T>> get_plugin() const {
            return get_resource<const Plugins>().and_then(
                [](const Plugins& plugins) { return plugins.get_plugin<T>(); });
        }
            ...
            resource_scope([&](Plugins& plugins) {
                plugins.get_plugin_mut<StateUpdater>()
                    ...
                    .transform([this](StateUpdater& updater) -> bool {
                        ...
                        auto update_system =
                            make_system_unique([](Res<NextState<T>> next_state, ResMut<State<T>> state) {
        ...
        template <typename F>
            requires requires {
                ...
                requires std::derived_from<
                    std::decay_t<std::tuple_element_t<0, typename function_traits<F>::args_tuple>>, AppRunner>;
                requires std::invocable<
                    F, std::decay_t<std::tuple_element_t<0, typename function_traits<F>::args_tuple>>&>;
            }
