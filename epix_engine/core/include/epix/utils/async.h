...
template <typename T>
struct RwLock {
    struct ReadGuard : private std::pair<std::shared_lock<std::shared_mutex>, const T*> {
        using base_type = std::pair<std::shared_lock<std::shared_mutex>, const T*>;

       private:
        ReadGuard(std::shared_lock<std::shared_mutex>&& lock, const T* ptr) : base_type(std::move(lock), ptr) {}
    ...
    struct WriteGuard : private std::pair<std::unique_lock<std::shared_mutex>, T*> {
        using base_type = std::pair<std::unique_lock<std::shared_mutex>, T*>;

       private:
        WriteGuard(std::unique_lock<std::shared_mutex>&& lock, T* ptr) : base_type(std::move(lock), ptr) {}
   ...
   public:
    template <typename... Args>
        requires(!((sizeof...(Args) == 1) && (std::same_as<RwLock<T>, std::decay_t<Args>> || ...))) &&
                std::constructible_from<T, Args...>
    RwLock(Args&&... args) : m_value(std::forward<Args>(args)...) {}
    RwLock(const RwLock&) = delete;
    RwLock(RwLock&& other) noexcept : m_value((other.m_mutex.lock(), std::move(other.m_value))) {
    ...
    ReadGuard read() const { return ReadGuard(std::shared_lock<std::shared_mutex>(m_mutex), &m_value); }
    WriteGuard write() const { return WriteGuard(std::unique_lock<std::shared_mutex>(m_mutex), &m_value); }
...
struct MultiGuard<std::tuple<Vs...>, L> {
   ...
   public:
    MultiGuard(std::tuple<Vs*...>&& values, L&& lock) : m_values(std::move(values)), m_lock(std::move(lock)) {}
    ...
    std::tuple<const Vs&...> operator*() const& {
        return std::apply([](const Vs*... vs) { return std::tie(*vs...); }, m_values);
    }
};
...
auto scoped_write(const RwLock<Ts>&... lock) {
    ...
    return MultiGuard<std::tuple<Ts...>, decltype(final_lock)>(std::make_tuple(&lock.m_value...),
                                                               std::move(final_lock));
}
template <typename... Ts>
auto scoped_read(const RwLock<Ts>&... lock) {
    MultiLock final_lock(lock.defer_read()...);
    return MultiGuard<std::tuple<const Ts...>, decltype(final_lock)>(std::make_tuple(&lock.m_value...),
                                                                     std::move(final_lock));
}
}  // namespace epix::utils::async
