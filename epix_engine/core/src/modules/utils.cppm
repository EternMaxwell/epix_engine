...
using ::internal::int_base;
using ::internal::make_channel;
using ::internal::Receiver;
using ::internal::Sender;
}  // namespace epix::core

export template <std::integral T>
struct ::std::hash<::internal::int_base<T>> {
    size_t operator()(const ::internal::int_base<T>& v) const { return std::hash<T>()(v.get()); }
};
export template <typename T>
    requires requires {
        typename T::value_type;
        requires std::derived_from<T, ::internal::int_base<typename T::value_type>>;
    }
struct ::std::hash<T> {
    size_t operator()(const T& v) const { return std::hash<::internal::int_base<typename T::value_type>>()(v); }
};