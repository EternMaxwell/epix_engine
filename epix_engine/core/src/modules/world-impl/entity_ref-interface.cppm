        ...
        return world_->components().get(type_id).and_then(
            [&](const ComponentInfo& info) -> std::optional<std::reference_wrapper<const T>> {
                ...
                if (storage_type == StorageType::Table) {
                    return world_->storage().tables.get(location_.table_id).and_then([&](const Table& table) {
    ...
    std::optional<Ref<T>> get_ref() const {
        ...
        return world_->components().get(type_id).and_then([&](const ComponentInfo& info) {
            ...
            if (storage_type == StorageType::Table) {
                ...
                return world_->storage().sparse_sets.get(type_id).and_then([&](const ComponentSparseSet& cs) {
   ...
   public:
    ...
    std::optional<Mut<T>> get_mut() {
        ...
        return world_->components().get(type_id).and_then([&](const ComponentInfo& info) {
            auto storage_type = info.storage_type();
            if (storage_type == StorageType::Table) {
                return world_->storage_mut().tables.get_mut(location_.table_id).and_then([&](Table& table) {
                ...
                return world_->storage_mut().sparse_sets.get_mut(type_id).and_then([&](ComponentSparseSet& cs) {
